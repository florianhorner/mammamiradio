...
3. Hear Demo Radio with no provider keys
4. Add `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` from **Motore → Setup → AI hosts** to unlock live AI hosts
5. Review **Home context preview** and mute any entity the hosts should never use. Supervisor Home Assistant access is automatic in add-on mode, but filtered home context is useful only after an AI host key is ready
