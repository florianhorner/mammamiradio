...
- **Host memories now come only from banter that cleanly streamed on the station timeline.** New listener theories and song reaction cues are extracted afterward from the final streamed script, so queued, skipped, fallback, or half-sent banter no longer teaches the station from an unfinished or discarded draft.
