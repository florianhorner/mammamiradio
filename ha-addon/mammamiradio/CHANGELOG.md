...
It plays before you tell it anything. The station now arrives with its own music, its own sound and its hosts already recorded, so a fresh install is a radio station within a second or two of opening it — before an AI key, before a speaker, before you have told it anything about your home. That meant taking something out: the add-on no longer pulls music off the open internet.

### Breaking

- **The add-on no longer downloads music from the internet.** `yt-dlp` is not installed in either channel. Live chart refresh, searching beyond your own library, and fetching a requested song are unavailable; searching the music you already have still works, and a song request the station cannot fetch becomes a shout-out on air. Each of those operations explains itself in plain words instead of failing.
- **The Jamendo client ID option is gone from the configuration screen.** A saved ID is moved once, on the next start, into the owner-only `/config/secrets.env`. Jamendo is now set up under **Motore → Setup → Music sources**, and you no longer need an ID at all — the station brings its own, with **Use own ID** still available.
- **A new install no longer starts with home context switched on.** It stays on coarse daylight and one plain weather source until you have heard the station play, seen exactly what the hosts would be handed, and chosen. An existing station keeps the home behaviour it had.

### New

- **First Listen plays on this device.** The setup tab starts the station and asks whether you heard it. No AI key, no Home Assistant speaker, no media integration required first. Marco and Giulia introduce each step in their own voices, transcripts included, and every step shows its state with a shape as well as a colour. Finished setup returns you to the control room, with repair and retesting under **Motore → Setup**. The whole flow works on a phone.
- **Twelve tracks come with the station.** Forty-seven minutes of attribution-only music, six from Incompetech and six from Jamendo, each one auditioned and approved before it shipped. The listener page credits the song that is playing and lists the full collection, and the licence travels with the music to external players.
- **Hosts worth listening to without an AI key.** Twenty-one reviewed Marco and Giulia breaks ship with the station and play without repeating.
- **A sound of its own.** The Modern Night Drive imaging pack carries the station ID, sweeper, cues, beds and ad scenes, with separate opening, middle and closing bumpers in every ad break.
- **Keep this.** Save a host break, ad, news flash, station ID, sweeper or time check permanently, with a link that never expires and an **Archivio** shelf to take one back off.
- **Your music folder is picked up without a restart**, with a **Scan now** action, counts, and a way forward when something is unreadable.

### Fixed

- An empty music crate no longer loops the recovery clip about once a second.
- A song and the break after it are joined exactly once, and a restart cannot resurrect a half-joined fragment.
- A Home Assistant safety interrupt is followed by real music, not recovery audio.
- A speaker pointed straight at the stream no longer stutters between segments.
- Changing the music source no longer skips the songs it deliberately kept.
- Jamendo connects again, keeps a discovered track when only its links change, and says in plain words what went wrong instead of counting failures.
- Clear pool explains that it never deletes your files, and points at what to do instead.
- Song requests stay with the artist you named, and a request already read on air is not offered for resending.
- Now Playing names only the hosts who actually spoke, and the hosts no longer guess how many people are home.
- Household names stay off host-facing surfaces, and a generated admin token is no longer printed into the container log.
- The stream's genre label carries your tagline instead of the hosts' internal writing brief.
- "Tracks in Rotation" counts the records the station has loaded, not the plays this session.
- Ad breaks show the brands that are airing, in order, in the now-playing strip and in your phone's media controls.
