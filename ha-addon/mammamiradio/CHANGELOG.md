...
- **Failed or stale companionship copy cannot return later.** Generation, voice, quality, queue, purge, stop, overflow, and playback failures permanently settle the one-shot cue; an old-epoch cue is dropped before its next audio byte and consumption waits for a listener queue to accept audio. English/Italian connection-arrival claims get one repair and then deterministic safe copy, while packaged spoken recovery assets are transcript-reviewed and hash-pinned so changed or unlisted welcome/banter clips fail closed.
...
- **Installing or updating the add-on works again.** The add-on repository was naming a version whose image had not been published yet, so Home Assistant offered an update that could not download, and a fresh install failed outright. The repository now names only a version you can install, and a daily check raises a flag if that ever slips. A station already playing was never affected: Home Assistant downloads the new image before stopping the running one, so a failed update left the music going.
