...
- **Restart handoff scratch files no longer build up in `/data/cache`.** On startup the add-on now clears stale restart-handoff `.tmp` files left behind by an interrupted update, while keeping the published manifest and finished music handoff files intact.
