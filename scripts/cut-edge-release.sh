...
# builds an image when a commit touches the add-on image or its media-proof inputs.
# When the tip commits are unrelated tests/docs/CI-only, no :<sha> image exists
...
IMAGE_PATHS=".github/workflows/addon-build.yml ha-addon mammamiradio proof/media scripts/media-proof.py scripts/starter-catalog.py scripts/validate-addon.sh scripts/validate-starter-media.py tests/media tests/playlist/test_jamendo_transient.py tests/playlist/test_legacy_media.py tests/scheduling/test_queue_mutations.py tests/web/test_streamer_routes_extended.py pyproject.toml requirements.txt requirements-dev.txt radio.toml model_registry.toml"
