...
Add to Review Discipline: every audio-delivery feature test must cover:
1. Normal / happy path
2. Empty fallback resources (no canned clips, empty norm cache, no assets)
3. Post-restart state (flag files, session flags, persisted state)
Missing test for scenario 2 or 3 = untested blast radius, do not ship.
**Files:** `CLAUDE.md`
...
1. ffmpeg filter chain equalizer count ≤ 2 (assert in normalizer.py source)
