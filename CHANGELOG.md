...
- **README rewritten as a 30-second product pitch**: `CLAUDE.md` now lists four proposal principles every change should pass. New principle: a new reader should understand the product in 30 seconds or less, and the source tree should support the same clarity.
...
- **`## Health Stack` in CLAUDE.md** — `make check` and the `/health` skill share the same authoritative tool list (mypy, ruff, pytest, vulture, shellcheck), so the dashboard never prompts for tool detection again.
...
- **Listener brand wordmark — golden "Mi" accent was uncoloured since PR #235**: `<span class="mi">Mi</span>` in the nav brand depended on a `.nav-logo h1 .mi` rule whose ancestor selector never matched the new Volare DOM. The "Mi" rendered in the default cream colour instead of golden sun (per CLAUDE.md "Protected UI elements"). Restored by extending `.mmr-brand em` to also match `.mmr-brand .mi`.
...
- **Dashboard: extract inline CSS/JS into `/static/` (PR #203)** by [@ashika-rai-n](https://github.com/ashika-rai-n). Moved ~575 lines of inline CSS and ~815 lines of inline JS from `dashboard.html` into static files, with a one-line fix to `_inject_ingress_prefix` so `src="/static/..."` gets rewritten behind HA Ingress alongside `href="/static/..."`. Commit [`2028d40`](https://github.com/florianhorner/mammamiradio/commit/2028d408499cd98b15c82a39a5cd3912cdfbb1d9). The extraction pattern informed the later design-system consolidation.
