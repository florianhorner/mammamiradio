...
- **Time checks now hand back to the music with the same branded transition as every other host segment.** They previously used a bare synthesized whoosh with no musical motif — the one moment on air that sounded noticeably different from the rest of the station's imaging.
