...
- **Track B v2.11.0 groundwork (Phase 1 + Phase 2)** — listener-request endpoints now live in their own module (`mammamiradio/web/listener_requests.py`), extracted from `streamer.py` with no behavior change. Every `pending_requests` record now carries the four fields the upcoming dedication state machine and shared sidebar need: a canonical `request_id` (uuid4), an initial `status` of `queued`, an `evict_after` slot reserved for terminal-state TTL, and a `submitter_ip_hash` (HMAC-SHA256 keyed on `ADMIN_TOKEN`). The per-IP rate-limit dictionary migrated to that hash so no record carries a raw IP. `GET /public-listener-requests` now exposes `request_id` and `status` for the listener sidebar; `submitter_ip_hash` and `evict_after` stay server-only. Admin `GET /api/listener-requests` adds the same plus `evict_after`. `POST /api/listener-requests/dismiss` accepts both the legacy `ts` id and the new canonical `request_id`, so v2.12 listener UIs can migrate without a coordinated cutover. State-machine transitions, per-IP cap-of-3 enforcement, eviction sweep, audio cue, and the listener sidebar render land in follow-on PRs — this PR is purely additive.
