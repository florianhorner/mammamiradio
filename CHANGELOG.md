...
- **Your own music is the base again, and it airs under its real name.** On a Home Assistant Green with thirty songs in `/data/music`, the station was still playing the bundled starter catalog most of the time — about one local play in eight — because the scanner added the files without telling selection the crate had changed, and selection kept walking the starter bag in order. Untagged filenames also reached Now Playing and the hosts as `Unknown – <filename>`, with every local track timed as three and a half minutes. The scanner now reads title, artist, and duration from the file tags (falling back to the `Artist - Title` filename convention), marks the crate as local whenever those files are present, and lets weighted selection treat them as the base while still cycling the starter bag without early repeats. The recovery bridge that inserts a starter when the queue is empty asks for starter media explicitly, so fixing the mix does not hand it a local track and fall through to a canned voice clip.

- **Reading your song tags no longer costs the station a duration — or the charts.** Three follow-ups to the change above. The station re-read the tags of every file in your music folder once a minute, whether or not anything had changed, which on a full library left the box little time for anything else; tags are now read once per file and again only when that file changes. When a tag read timed out, the song's real length was replaced with a flat three and a half minutes, so Up Next and the runway drifted; a failed read now changes nothing and is simply retried. And on a station running the charts, a single file in the music folder switched the crate to local and quietly stopped the charts from ever refreshing again — your own music still leads the rotation, but the charts keep refreshing underneath it.
