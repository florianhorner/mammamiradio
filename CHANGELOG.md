...
- **Security: the optional music-download helper now requires a patched version.** The standalone `external-media` extra accepted a version of the `yt-dlp` helper that carries a known flaw (CVE-2026-55404), where a crafted download could hand a command to the machine running it. Fresh installs now refuse anything below the fixed release. Home Assistant add-on users were never affected, because the add-on does not ship this helper at all and its build refuses to start if the helper is present. If you already installed the extra on a standalone setup, reinstall it to pick up the fixed version: a version floor only applies when the helper is installed, not to one already sitting on your machine.
