...
- **README "Never Crashes, Always Plays" table** now matches actual `fetch_startup_playlist()` priority order. The `MAMMAMIRADIO_ALLOW_YTDLP not set` and `jamendo_client_id not set` rows previously claimed fallbacks that the code didn't actually do. Now describes the real chain: charts (gated on yt-dlp) → Jamendo (gated on client_id) → local `music/` → bundled demo assets → `DEMO_TRACKS`. Closes finding 3 of the codebase-issues task list.
