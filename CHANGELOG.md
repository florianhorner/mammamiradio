...
- **`tests/scheduling/test_producer_unit.py:573`** — fix stale assertion that pinned the pre-cathedral demo-assets path. The test asserted `"check demo_assets/banter/"` but `producer.py:1536` now logs `"check assets/demo/banter/"` (post-cathedral). PR #279 updated the producer message but missed the test; this restores the assertion. (Worth a separate followup: the `quality.yml` workflow has been silently swallowing pytest non-zero exit codes — every CI run since #279 said "success" while showing `1 failed` in the test output. Tracked separately.)
