...
- **`OPENAI_SCRIPT_MODEL` env-var operator toggle** — switches the OpenAI model used for script-generation fallback (banter/ads/news/transitions) when Anthropic is unavailable. Default `gpt-4o-mini`; production behavior unchanged. Adds structured `openai_script_fallback` log events (model, caller, latency_ms, prompt/completion tokens, fallback_reason, json_ok) so an evaluation run can compare candidates from logs alone. Ships with an offline harness at `scripts/eval_openai_script_model.py` + prompt corpus at `scripts/eval_fixtures/openai_script_prompts.json` for side-by-side comparison. Does NOT touch TTS — `gpt-4o-mini-tts` is unchanged.
