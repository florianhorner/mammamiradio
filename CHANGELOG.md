...
It plays before you tell it anything.

This is a major release. Mamma Mi Radio now arrives with its own music, its own sound, and its hosts already recorded, so a fresh install is a radio station within a second or two of opening it — before an AI key, before a speaker, before you have told it anything about your home. Getting there meant taking something out. The add-on no longer pulls music off the open internet. It brings a crate of its own instead, and it can tell you where every record in it came from.

### Breaking

- **The add-on no longer downloads music from the internet.** `yt-dlp` has moved to an optional `external-media` package and is not installed in either add-on. Refreshing live charts, searching beyond your own library, and fetching a requested song are unavailable there; searching the music you already have still works, and a song request the station cannot fetch becomes a shout-out on air instead of a stalled download. Each of those operations answers with a plain explanation of what is and is not possible rather than a failure. If you run the station outside Home Assistant and want the old behaviour, install it with the `external-media` extra and set `MAMMAMIRADIO_ALLOW_YTDLP=true`.
- **The Jamendo client ID has left the add-on configuration screen.** An ID you had saved is moved once, on the next start, into the owner-only `/config/secrets.env`, and the field is gone from the options. Jamendo is now set up in the control room under **Motore → Setup → Music sources** — and you no longer need an ID at all, because the station brings its own. If you would rather use your own, **Use own ID** is still there.
- **A new Home Assistant install no longer starts with home context switched on.** The setting is absent from the add-on defaults rather than set to on, so a fresh station stays on coarse daylight and one plain weather source until you have heard it play, seen exactly what the hosts would be handed, and chosen. A station that already had home context keeps it.

### It plays before you tell it anything

- **First Listen now plays on this device.** A fresh install starts the station in the setup tab and asks whether you heard it. No AI key, no Home Assistant speaker, no media integration first: the proof happens in the browser tab you already have open, in about half a minute. Speakers and the media source are still there for later, they are simply no longer a prerequisite for finishing setup.
- **Marco and Giulia talk you through it.** Each step carries a short recorded example in the hosts' own voices, around twelve seconds, played only when you ask for it. Nothing waits on them: a clip is fetched at the moment you press play, and a slow or missing one leaves the written guidance and the setup exactly where they were. Every clip has its transcript underneath for reading instead of listening, and the hosts open the way a radio show opens, by name.
- **Every step says where it stands.** The status labels on play-it-here, sound check, home details and AI setup were being written but never shown, so a step that needed a second look read the same as a finished one. They are visible again, and each state carries a shape as well as a colour — a check, a triangle, a cross — so the difference reads without depending on colour.
- **Setup tells the truth about what is playing.** A station running on backup audio reads amber and says so, instead of presenting recovery audio as proof that your music is ready. Live charts, Jamendo, your own files, the bundled crate, recovery audio and custom rotations each get their own plain-language status, with the raw wording kept in Technical details where it belongs.
- **Finishing setup gives you the control room back.** First Listen stays in front only while it is genuinely needed; once you are through, you land in Scaletta, and repair and retesting live under **Motore → Setup**. Every exit puts you on a valid tab and stops both audio players. The whole flow works at phone width, including tabs, playlist rows and search results.
- **Setup no longer points you at an add-on you are not running.** A standalone station now names the two settings it reads and says the radio plays without a home connection, so the step can be skipped instead of dead-ending. And a station that cannot reach Home Assistant at all can still walk the guided path and reach the AI-key step.

### The station brings its own records

- **Twelve tracks come with the station.** Forty-seven minutes of music, packaged inside the app: six from Incompetech under CC BY 4.0 and six from Jamendo under CC BY 3.0, every one attribution-only, every one listened to end to end and approved by a person before it shipped. Anything carrying a NonCommercial, NoDerivatives or ShareAlike term cannot enter the collection.
- **Every song can say where it came from.** One manifest holds each track's source page and licence link, the file hashes before and after processing, what was changed and why, the duration, and the record of who approved it. The listener page credits the song that is playing and can open the full included list, and the licence and credit travel with the music on the feed any external player can read. A release cannot go out unless all twelve files, at least forty-five minutes of audio, the complete audition record and the size ceiling all pass.
- **A station with no AI key now has hosts worth listening to.** Twenty-one reviewed Marco and Giulia breaks — close to twenty-six minutes of finished audio — ship with the station and play without repeating. The English-led and Italian-first pools stay separate, the three breaks written about a specific song only air after that song, and the three that wink at the audience stay rare.
- **Mamma Mi Radio has a sound of its own.** The Modern Night Drive imaging pack: Neon Relay carries the station ID and sweeper, Velvet Horizon sets the atmosphere for the cues, beds and the nine ad scenes, and ad breaks now have separate opening, middle and closing bumpers so no two moments in a break sound alike. Forty-seven finished pieces, each tied to its own retained source and approved on a listening board before it aired.
- **Jamendo is an optional expansion, and it works again.** It stays off until you switch it on and confirm the non-commercial terms, then prepares at most one track at a time and slots it in after two of your own or bundled songs. The connection itself was broken and is fixed, discovered tracks are no longer thrown away when Jamendo rewrites its own links between calls, and you no longer need a Jamendo account to use it.
- **The Jamendo row says what is wrong instead of counting failures.** The card used to read "Preparing one Jamendo track" with a running total of skipped songs and no reason, so a source failing every attempt looked identical to one about to succeed. The count is gone. The row now states, in plain language, what went wrong and whether you need to do anything, while it is trying, between retries, and when it gives up. Two reasons carry a real fix: an ID Jamendo will not accept, and a working folder the station cannot use. Everything else says plainly that no action is needed, and a song that prepares correctly no longer carries the reason an earlier one was skipped.
- **Your own music folder is picked up without a restart.** Discovery is recursive, refreshes every minute, accepts the audio formats the station already handles, and adds a **Scan now** action with counts and a way forward when something is unreadable. A scan that comes back incomplete changes nothing about what is playing or queued. Drop files in; they turn up.
- **Clear pool no longer looks broken.** Clearing the rotation pool never deleted your own files, and the station reloads them within moments, so songs appeared to come back on their own. The button and its confirmation now say that plainly and point at what to do instead: ban a song to keep it out for good, or remove the file from the folder.
- **Sharing a song stays inside what the station owns.** Only a complete bundled track can be shared; anything else says so rather than quietly producing a clip.

### The show keeps playing

- **An empty music crate no longer loops the station ident every second.** With no bundled tracks, no local files and nothing else on the way, the station kept rebuilding its send-ahead cushion by bursting the recovery clip from the first syllable, roughly once a second. The clip now follows the same timeline as any other segment, files dropped into the music folder are picked up while the crate is empty, and Jamendo can start when there is no rotation to wait behind.
- **A song and the break after it are joined exactly once.** Never twice, never not at all. A partly built pair can be rolled back before anyone hears it, and the joined-but-unfinished piece is never written to the restart spool, so a restart cannot bring back a fragment as though it were a whole track.
- **A safety interrupt is followed by music.** When Home Assistant interrupted the show and the cache was cold, the station could fall through to recovery audio. It now adds a verified bundled song instead, and only when an interrupt actually emptied the queue.
- **A speaker pointed straight at the stream no longer stutters between segments.** The station runs a few seconds ahead of what you hear, so the short pause while a new ad, host break or station ID is prepared stays inside that head start instead of reaching the speaker.
- **Preparing music no longer competes with the show.** Paced transfers for an optional music source are held outside the audio work the station needs to keep the stream moving.
- **Changing the music source no longer skips the songs it kept.** Loading a new source keeps a little audio from the old one so the station never falls silent, and keeps the exact song a host has just promised a listener. Those kept songs were then refused at the moment they should have played: the announcement aired and the song did not follow, and on a station with nothing else ready the skipping reached for emergency audio. Kept songs now play, whether you switch sources, use Clear pool, or refill an empty music folder mid-show.
- **A steered stretch stops replaying the same few songs.** Point the station somewhere with Record Hunt and it favours what it found. When the hunt only resolved a handful of tracks, that favouring kept landing on the same ones and a song could return within half an hour. A found track now waits for the rest of its set before it can play again, and the wait is measured against the songs the station can actually pick.

### Keeping what the hosts do

- **You can now keep a moment before it disappears.** Everything the station recorded was on a timer: a shared clip deleted after a day, the written record after two weeks. So anything the hosts did worth hearing twice was gone before you could come back to it. The control room now has a **Keep this** button. Press it while the hosts are talking, or in the seconds after a break ends and the music comes back, and it is saved for good with a link you can share. It covers host breaks, ads, news, station IDs, sweepers and time checks. A song cannot be kept, and neither can a break that opens over the end of one: those recordings belong to whoever made them. A kept link never expires, and **Archivio** lists everything you have kept, with an undo on the confirmation, so you can take one back off the shelf.

### It says only what it knows

- **Now Playing no longer credits a host who was not on the break.** External players had no way to tell who was speaking, so they listed every host the station has configured, including the guest on breaks he never joined. The live feed now names only the hosts who spoke, and names nobody on a pre-recorded clip rather than guessing.
- **Hosts no longer invent how many people are home.** The station knows whether someone is home or away. It does not know how many, or which room. That gap could come out on air as a confident guess. The hosts are now told not to fill it in, in either language, and the experimental setting that names your home's mood follows the same rule.
- **Household names and admin tokens stay off shared surfaces.** Private resident labels are replaced with generic ones wherever the hosts can see them, and a generated admin token is no longer printed into the container log when it cannot be saved. The startup message explains how to set one that survives a restart instead of showing you the value.
- **Song requests stay with the artist you named.** Open-ended requests no longer cue an unrelated search result: matches are checked against the requested artist or song, titles are cleaned of video-site clutter, and a guest credit no longer splits one recording into two. The listener page follows the request while it searches, then shows the verified match or a clear next step. It stops watching after ten minutes instead of forever, eases off when the station is unreachable, and hands the form back honestly. A request that has already been read on air is told apart from one the station has lost, so nobody is invited to send a song they just heard.
- **"Tracks in Rotation" counts the records, not the plays.** The listener stat showed how many songs had played this session, which is not what the label promised. It now shows how many the station has loaded, and says so plainly when the crate is still empty.
- **The stream shows your tagline where players expect a genre.** That label was being filled with the station's internal writing brief, the notes that tell the hosts how to sound. Those notes were never meant for anyone's screen. It now carries the public tagline from your brand settings, and is left off entirely when no tagline is set.

### Also in this release

- **Ad breaks name their brands while they air.** The fictional brands in the current break appear in the now-playing strip and in your phone's media controls, in the order they run, with a compact bilingual receipt of the spots that completed. A break is credited only once a real ad reaches its end and every part of it was accepted by a listener.
- **Host breaks stop settling into the same rhythm.** Banter follows structured exchange shapes chosen with the line-up in mind rather than drifting into the same back-and-forth every time, and the station records which shape it asked for and which backup fired, so a stock-copy recovery stays honest.
- **The listener page costs less to keep open all evening.** Unchanged status now comes back empty instead of resending the whole picture, a visible live tab refreshes every three seconds, hidden and stopped tabs back off, and the ages on the Casa card are anchored to a clock that cannot drift. A missing home moment reads as unknown rather than appearing current.
- **Mamma Mi Radio is spelled correctly on the one screen where it was not**, the First Listen recovery step, and a repo-wide guard now fails if the joined spelling ever comes back.
...
- **A page you can send someone.** There is now a public page that plays the station before it explains it: one click starts a real thirty-second segment, and the sensor panel arrives afterwards as the reveal. One of its four moments uses only the sun and the weather, which is all a fresh install can share. It reads no home data and sends nothing anywhere.
...
- Listen and Pause controls, paused or reconnecting status, and the public schedule now reflect the live station on desktop and mobile.
...
- Connected Home now requires usable context. Home Assistant users can turn off host context or adjust its refresh interval.
...
- **Durable listener memories now wait for a clean banter stream.** The station still lets hosts use existing listener memory while writing a break, but new listener theories and song reaction cues are extracted afterward from the final streamed script. Queued, purged, skipped, fallback, or half-sent banter no longer writes durable listener or song-cue memory.
- **The two language modes now do what they say.** With Super Italian Mode on, the hosts speak fully in Italian — no more English asides slipping through. In the default mode, the hosts now speak about 70% English with real Italian moments — including whole Italian sentences — instead of only scattered Italian words, and news flashes and ads follow the same mix rather than staying all-Italian inside an English-led show. The admin toggle's description now says plainly what each mode does.
- **Hosts push through a long thought instead of shrugging into filler.** When a host break runs longer than the writing budget allows, the station now retries once with more room — first with its main writer, then with the backup — instead of quietly airing one of a handful of generic filler lines. The budget itself also grew to match how much the hosts actually have to say (home context, running jokes, listener callbacks), each attempt gets breathing room proportional to how long the reply is allowed to be, and the whole thing stays bounded: past a hard time limit the station stops retrying and falls back exactly as before, so the music never waits on a chatty host. Operators watching logs get an early "budget pressure" warning while breaks still succeed, plus honest per-attempt records of what was retried and what it cost.
...
- **"Clear pool" now actually empties the rotation.** The clear-pool button in the Rotazione tab used to fail with an error; it now clears every track from the pool (and the pinned song), and the music that's already playing finishes before the station waits for you to add a new source — no dead air.
- **Admin panel visual cleanups.** The "Shuffle" button shows its proper icon instead of a broken symbol, the host personality sliders fill as a clean thin track instead of a tall block of colour, the doubled hairline under section headers is now a single rule, and the gold accent on the live console runs cleanly into its rounded corners instead of leaving a hard notch.
- **The admin producer deck takes up less space when nothing is playing.** While the station is waiting for the first segment, the console now collapses to a compact bar — the "Waiting for signal" title shrinks and the progress row disappears — so the section tabs and live queue are visible without scrolling. When audio starts the console expands back to its full height. The four air-next trigger buttons move to a single horizontal row on wider screens. Touch targets on the filter chips and section tabs are now the full 44 × 44 px — making the panel usable one-handed on a phone.
- **Some brand jingles now play their intended sound instead of a generic chime.** A handful of fictional ad brands had a sound cue in their configuration that didn't match any real effect, so their opening jingle silently fell back to a plain default chime on every airing. Those brands now use real effects, and a check now catches this class of typo before it ships. Ad scripts also stop reusing a music mood's name (like a jazzy or romantic vibe) as a sound-effect cue.
...
- **Seven new fictional sponsors join the ad rotation.** The hosts now have more absurd brands to riff on: a coffee-bar notary, a vengeful alarm clock, an emotional condominium elevator, a rug that remembers everything, a gelato shop that picks flavours by the weather, and a pair of smart fridges from the same fake conglomerate — one that judges your midnight snacking and one that eggs it on. Every brand is invented; none names a real company.
- **Mamma Mi Radio is now a real Home Assistant media player.** A new HACS integration registers the station as a proper `media_player` entity, so it shows up first-class in the Home Assistant dashboard card picker with no YAML, and your automations and voice assistant can control it: play, stop, and skip to the next segment (controls work automatically on Home Assistant OS; a remote or Docker install uses the station's admin token). It shows what is on air — title, artist, and artwork, with the station logo during host breaks and ads. Install it from HACS as a custom repository, then turn off the add-on's new **On-air media player push** option so the two do not collide. See `docs/integrations/ha-integration.md`.
- **Hand the live station to any speaker in the house.** The integration now exposes Mamma Mi Radio as a Home Assistant media source, so a `media_player.play_media` call or a Follow Me Music-style handoff can start the live station on any real speaker — pick **Mamma Mi Radio Live** from the media browser, or use `media-source://mammamiradio/live` as the `media_content_id`. Home Assistant serves the audio through its own stream proxy, so the speaker only needs to reach Home Assistant, not the add-on directly, and `media_player.mammamiradio` stays the station control surface.
- **Steer the music by mood, and the hosts notice.** Tap an era or a vibe in the admin panel and the station re-aims its own programming toward it, weaving that direction into the live rotation. The on-air host picks it up as a feeling rather than a setting — "someone's got a soft spot for the 80s tonight" — so it always sounds like the station read the room, never like it ran a command. The steer sticks across a restart, clears with one tap back to automatic, respects your banned songs, and never interrupts what's playing.
...
- **A slow smart home can no longer stall the station.** When the hosts reach for live home context (for a comment about the house or a weather flash), a sluggish or unresponsive Home Assistant used to be able to hold up the next segment. The station now gives that lookup a short deadline and, if it runs over, airs on the most recent home reading (or none) instead of pausing — so the music keeps flowing no matter how the house is doing. The first reading after start-up still gets extra time to load fully.
...
- **The hosts keep talking when the station falls back to its backup writer.** When the main scriptwriter was unavailable and the station switched to its backup writer, every backup attempt was being rejected before it aired — so banter, ad reads, news flashes, and transitions all dropped to generic stock copy. The backup path now sends the request in the shape the newer writer expects, so the DJs keep their real chatter while the main writer is resting. The admin key check was tripping over the same thing — it could report a perfectly good backup-writer key as broken — and now reports it correctly.
...
- **PR landing is now mechanized and pinned to the reviewed code.** Branch protection requires branches to be up to date before merging, and maintainer merges go through `scripts/land-pr.sh`, which updates the branch when needed (CI re-runs on the integrated state) and arms GitHub auto-merge locked to the exact head commit that was verified — a later push cancels the landing instead of shipping unseen changes. A read-only settings tripwire (`scripts/check-merge-gate.sh`, part of `make pre-release`) catches drift in the underlying repository settings. Correction: the automatic Dependabot rebase nudge was later retired because GitHub's workflow actor could post the command but Dependabot rejected it, creating repeated comments and CI churn instead of hands-free landings.
...
- **Classic Italian era source** — three new admin playlist buttons (`Anni '70`, `Anni '80`, `Anni '90`) let the operator load era-themed Italian music via yt-dlp search. Queries target cantautori (70s), 80s pop, and 90s pop with representative artist hints. Each fetched track is stamped with a `year` hint (`1975` / `1985` / `1995`) derived from the era, not the upload date. Persists through restart: `kind="classic"` round-trips through `playlist_source.json` and restores correctly. If yt-dlp is disabled, the button shows a toast error instead of silently falling back. `classic://italian/{era}` URL scheme mirrors the existing `jamendo://` dispatch pattern.
- **Era-aware playlist visualization** — admin playlist rows now show a 36 px album art thumbnail, a decade badge (`'70` / `'80` / `'90` / year-derived), and a source chip (`classici` / `charts` / `jamendo` / `locali`). Empty-art rows show a warm placeholder. Drag-drop reorder is unchanged. `updatePl()` refactored from string concatenation to template helpers (`trackThumb`, `eraLabel`, `sourceChip`); `loadPlaylistSource()` refactored from a hardcoded ternary to a `_SOURCE_DISPATCH` map.
- **Playlist serialization expanded** — `/status` `playlist` array now includes `album_art`, `source`, `year`, and `youtube_id` per track so the admin UI can render thumbnails and badges without extra round trips. Previously only `title`, `artist`, `display`, `spotify_id` were serialized.
- **Jamendo in HA add-on config** — `jamendo_client_id` option is now exposed in the Home Assistant add-on UI. Set it to enable CC-licensed music from Jamendo; leave empty to keep the source off. Previously the env var was never passed into the container.
- **Secret-safe provider check endpoint** — admin-only `POST /api/setup/provider-check` actively probes the live Anthropic key, OpenAI chat key, and OpenAI TTS key with tiny requests, returning only configured/ok/status/error-category fields. This makes it clear whether HA Green is using a different key than local `.env` without exposing key values.
...
- **Full imaging architecture** — music-to-voice and voice-to-music boundaries now get short branded transition stings, sweepers pick up motif underlays, and banter/news can sit over ducked talk beds for a more continuous station feel. The feature is enabled automatically, configurable via the new `[imaging]` block in `radio.toml` (`bed_volume_db`, `use_music_queue_for_beds`), and works without operator assets by generating FFmpeg stings and beds when no matching files exist under `mammamiradio/assets/imaging/`.
...
- **Admin control room reads as espresso warm-brown again.** PR #298 raised four shared `tokens.css` values (`--surface`, `--surface-hover`, `--surface-strong`, `--line-strong`) to make listener cards visible — but admin consumes the same tokens, and v2.11.0 shipped with admin washed out to taupe. Tokens reverted to Pi-baseline values; listener cards keep PR #298's brighter values via inline overrides on `.mmr-stage`, `.mmr-np-bar`, `.btn-ghost`, `.mmr-schedule`, `.mmr-dedica`, `.mmr-about-card`. Repo UI guardrails now protect against re-regression.
- **Banter and ad segments no longer cut off mid-sentence on Pi.** `concat_files` was using `anullsrc=r=48000:cl=stereo,atrim=duration=<secs>` to insert silence gaps between voice parts. On Pi aarch64, `atrim` does not reliably terminate the infinite `anullsrc` stream, causing the `concat` filter to produce output shorter than the sum of its inputs. Replaced with `aevalsrc=0|0:d=<secs>:s=48000:c=stereo`, which generates exact-duration silence without `atrim` — the same pattern used elsewhere in the normalizer. Regression introduced in v2.11.0.
...
correctly at rest on every viewport we test on, the admin panel is fully in
...
- **Jamendo `country` and `order` filters — Italian-trending music as the default Jamendo source.** Two new fields in `[playlist]`: `jamendo_country` (ISO 3166-1 alpha-3, e.g. `"ITA"`, `"DEU"`, `"FRA"`; empty disables the filter) and `jamendo_order` (`popularity_week` | `popularity_month` | `popularity_total` | `releasedate_desc` | empty for default). Both are also overridable via `JAMENDO_COUNTRY` / `JAMENDO_ORDER` env vars, validated at config load (3-letter uppercase for country, known-set for order). The Jamendo API URL builder threads them as `country=` and `order=` query params; the persisted `jamendo://playlist?...` round-trip URL encodes them so reload via `/api/playlist/load` reproduces the same fetch (round-trip test guards). `radio.toml` ships with `jamendo_country = "ITA"` and `jamendo_order = "popularity_week"` — Italian-trending music is now the actual Jamendo result, not "any-country pop". Same engine + different `country=` is the foundation for future country-specific radio "skins" (mammamiradio/ITA → faux-german-radio/DEU, etc.). Replaces the yt-dlp charts dance for the most common path: properly licensed, no `MAMMAMIRADIO_ALLOW_YTDLP` toggle. Python defaults stay empty for backward compat — existing radio.toml files without these fields get the prior behavior.
- **README rewritten as a 30-second product pitch**: repo guidance now lists four proposal principles every change should pass. New principle: a new reader should understand the product in 30 seconds or less, and the source tree should support the same clarity.
- **Repository restructure plan**: a file-by-file doc maps the move from 27 flat files to 7 subpackages (`core`, `audio`, `playlist`, `hosts`, `home`, `scheduling`, `web`), mirrors the test tree, consolidates root docs under `docs/`, and rewrites README to a 30-second pitch.
- **Regia Screen 2 — live queue view**: queue rendering from `queued_segments`, break-structure card, skip-current and purge-all controls, inline search against `/api/search`, and per-item removal via the new `POST /api/queue/remove` endpoint. Replaces the browser `confirm()` panic dialog with an in-page CSS overlay that doesn't drop the stream. Drag-to-reorder is deferred (asyncio.Queue is not random-access). The Regia design language now lives on `/admin` — see Removed below for the standalone `/regia` route cleanup.
...
- **Health Stack guidance** — `make check` and the `/health` skill share the same authoritative tool list (mypy, ruff, pytest, vulture, shellcheck), so the dashboard never prompts for tool detection again.
...
- **Content-based asset fingerprinting** for `/static/*.css` and `/static/*.js`. Visual fixes now invalidate stale browser URLs without a package version bump — the URL itself changes when the file content changes, so `radio-itali-v4` style cache-name bumps stop being load-bearing for ordinary CSS edits.

### Changed

- **`mammamiradio/` subpackaged into seven subpackages**: 25 flat python files reshuffled into `core/` (config, models, capabilities, setup_status, sync), `audio/` (normalizer, audio_quality, tts, voice_catalog), `playlist/` (playlist, downloader, song_cues, track_rationale, track_rules), `hosts/` (scriptwriter, persona, context_cues, ad_creative), `home/` (ha_context, ha_enrichment), `scheduling/` (producer, scheduler, clip), and `web/` (streamer, og_card, plus templates/ and static/). HTML templates moved out of the package root into `web/templates/`. The whole `static/` tree moved to `web/static/`. `assets/` (demo MP3s + SFX, logo) lives under the package. `tests/` mirrors the source tree exactly (61 test files into nine subpackage dirs). Used `git mv` everywhere to preserve history. Public addon entrypoint `mammamiradio.main:app` is unchanged. New `docs/REPO_MAP.md` answers "I want to fix X — where do I look?" in one table. README's first viewport was rewritten to a 30-second pitch with three navigation links. **Migration note for any out-of-tree script that imports modules directly:** flat paths like `mammamiradio.config`, `mammamiradio.streamer`, `mammamiradio.playlist`, etc. no longer resolve; rewrite to subpackage paths (`mammamiradio.core.config`, `mammamiradio.web.streamer`, `mammamiradio.playlist.playlist`, …) per `docs/REPO_MAP.md`. The HA addon entrypoint and Docker invocations are unaffected.
- **Repo root reduced to four top-level files; everything else moved under `docs/`**. All moves used `git mv` to preserve history.

  **Files kept at root** (one viewport, one job each):
  - `README.md`
  - `CONTRIBUTING.md`
  - repo agent guidance
  - `CHANGELOG.md`

  **Moved to `docs/` (top level):**
  - `ARCHITECTURE.md` → `docs/architecture.md`
  - `OPERATIONS.md` → `docs/operations.md`
  - `TROUBLESHOOTING.md` → `docs/troubleshooting.md`
  - `CONDUCTOR.md` → `docs/conductor.md`
  - `AGENTS.md` → `docs/agents.md`
  - `STABILIZATION_LOG.md` → `docs/stabilization-log.md`
  - `TODOS.md` → `docs/todos.md`

  **Moved to `docs/design/` and `docs/runbooks/`:**
...
- **Local `music/` files are now used as a startup source when yt-dlp is disabled and Jamendo isn't configured**: `fetch_startup_playlist()` previously logged a warning and silently fell through to bundled demo assets / `DEMO_TRACKS` when `allow_ytdlp=False` + no `jamendo_client_id` + local MP3s in `music/`. The warning even told operators to "set `MAMMAMIRADIO_ALLOW_YTDLP=true` to blend local tracks" — but yt-dlp is only needed to download charts, not to play files that already exist on disk. Now local MP3s in `music/` are loaded as a real source (`source.kind="local"`, `source_id="local_music_dir"`) right after the Jamendo branch, before any demo fallback. The previous behavior skipped local tracks that operators had already put on disk expecting them to play. Test `test_fetch_startup_playlist_local_music_warning_when_ytdlp_disabled` was inverted (it pinned the buggy behavior) and renamed to `test_fetch_startup_playlist_uses_local_music_when_ytdlp_disabled_and_no_jamendo`. New positive-case test `test_fetch_startup_uses_local_music_when_ytdlp_off_and_no_jamendo` added.
- **README "Never Crashes, Always Plays" table** now matches actual `fetch_startup_playlist()` priority order. The `MAMMAMIRADIO_ALLOW_YTDLP not set` and `jamendo_client_id not set` rows previously claimed fallbacks that the code didn't actually do. The corrected rows describe each source's *next* fallback (charts (gated on yt-dlp) → Jamendo (gated on client_id) → local `music/` → bundled demo assets); the built-in `DEMO_TRACKS` final fallback is documented in `docs/architecture.md` "Music sources" alongside the rest of the chain.
- **Charts source no longer impersonates local files when the charts API returns empty**: `_load_chart_source_tracks()` previously mutated an empty `chart_tracks` list by appending local MP3s via `_merge_local_music_tracks`. Callers saw a non-empty result and labeled it `kind="charts"` / `source_id="apple_music_it_top_100"`, persisting a misleading source kind to `cache/playlist_source.json`. With charts as the persisted source, restart would loop through the same chain and operator-visible source telemetry would lie. Now, when charts returns zero, `_load_chart_source_tracks` returns empty too, `load_explicit_source` raises `ExplicitSourceError` (honoring its "no silent fallback" docstring), and `fetch_startup_playlist` falls through to Jamendo / local / demo tiers, which label correctly. Two regression tests added: `test_charts_empty_but_local_present_does_not_lie_about_source_kind` (unit) and `test_fetch_startup_falls_through_to_local_when_charts_empty_and_ytdlp_on` (end-to-end).
- **Charts `source_id` numerical drift** (`apple_music_it_top_50` → `apple_music_it_top_100`): the persisted-source label said "top 50" but the URL fetches up to 100 tracks (`most-played/100/songs.json`, default `limit=100`). Pure label drift, no runtime impact, but a transparent migration in `read_persisted_source()` remaps the legacy ID on load so old `cache/playlist_source.json` files round-trip without warnings.
- **`tests/scheduling/test_producer_unit.py:573`** — fix stale assertion for the moved demo-assets path. The test asserted `"check demo_assets/banter/"` but `producer.py:1536` now logs `"check assets/demo/banter/"`. PR #279 updated the producer message but missed the test; this restores the assertion. The followup question this raised — "why was every CI run since #279 silently passing despite `1 failed`?" — is fixed by the CI swallow change below.
- **CI no longer silently swallows pytest failures.** `scripts/coverage-ratchet.py:run_coverage()` previously only failed when `result.returncode != 0 AND not modules` — pytest could fail tests, but as long as coverage parsing populated module rows the ratchet exited 0 and the workflow stayed green. Active since PR #279. The coverage ratchet now hard-fails on any non-zero pytest return code, so test-suite failures fail CI instead of being hidden by successful coverage parsing.
- **Listener cards visible at rest**: surface tokens lifted hard against the espresso body bg — `--surface` from `#251E19` to `#54453A` (~2.07:1 contrast), `--surface-strong` from `#362B25` to `#6E5B49` (~2.96:1), `--line-strong` from 16% to 32% cream — plus an inset top-edge highlight + drop-shadow on `.mmr-schedule`, `.mmr-dedica`, and `.mmr-about-card`. The earlier +5-luminance bump (`#2E2620` / `#40342C`) sat at 1.25:1 / 1.56:1, at or below the perceptual threshold, and live testing confirmed cards still read as void mid-page even when DOM rendering was correct. The new values land all listener panels above 2:1 so they register as panels at a glance, not page bg with a hairline border. Hex values mirrored in `docs/design/system.md`.
- **Listener page sections silently hidden on Safari and Chrome**: the fixed-position `body::before` glow overlay could be promoted into a compositor layer that occluded scrolled real-viewport content while full-page screenshots flattened it and made the sections appear visible. Removed the fixed overlay; the glow and grain stay in the normal page background. Anchor scroll margins added so sticky navigation cannot hide a target section after a hash jump.
- **Listener now-playing strip never shows "Session stopped"**: the idle state used to leak the internal segment label and uppercased type ("Session stopped" / "STOPPED") into the title and artist slots. Now renders "In pausa" with no artist sub-line, on both the DOM strip and Media Session metadata (lock screen / Bluetooth / CarPlay).
...
- **Service worker `/listen` precache restored**: a freshly-installed PWA can now open `/listen` cold-cache offline. Was missed in the network-first switch and broke instant audio for the offline edge.
- **Service worker catch-all branch for same-origin GETs**: brand assets (`logo.svg`, future webfonts, future static images) get network-with-cache-fallback handling instead of silently bypassing the cache. `CACHE_NAME` bumped so existing clients re-precache on next install.
- **Listener mobile — header overflowed phone viewport, broke vertical scroll, snapped on `In Onda` tap**: PR #235 (Volare Refined) renamed `.nav` → `.mmr-nav`. The pre-Volare `@media (max-width: 640px) { .nav-links { display: none; } }` rule was left targeting the old class name and never ported. On a 375 px phone the brand + 4 anchor links + In Onda pill (~540 px min-content, no `flex-wrap`) pushed the body past the viewport — sections scrolled to the right of the visible area (~80 % cut off) and tapping the live pill triggered a horizontal rubber-band snap. Three layered fixes in `mammamiradio/static/listener.css`: hide `.mmr-nav nav` and stack the footer to a single column at `<=600px`; add `min-height: 100svh` so iOS Safari address-bar collapse no longer cuts the bottom; add `overscroll-behavior-x: contain` to disable horizontal rubber-band on the body. Also bumped `.mmr-dedica-form-input` and `.mmr-dedica-form textarea` from `font-size: 14px` to `16px` so iOS Safari stops auto-zooming on focus and breaking the layout. Added `tests/test_listener_mobile_invariants.py` (three guards) so a future Volare-style refactor that drops the phone breakpoint, regresses to 14 px inputs, or removes `100svh` fails CI.
- **Listener brand wordmark — golden "Mi" accent was uncoloured since PR #235**: `<span class="mi">Mi</span>` in the nav brand depended on a `.nav-logo h1 .mi` rule whose ancestor selector never matched the new Volare DOM. The "Mi" rendered in the default cream colour instead of golden sun. Restored by extending `.mmr-brand em` to also match `.mmr-brand .mi`.
- **Mobile tap latency and tap-highlight flash on every interactive control**: `mammamiradio/static/base.css` now sets `-webkit-tap-highlight-color: transparent` on the universal reset and `touch-action: manipulation` on `button`, `a`, `[role="button"]`, `summary`, `label`, `select`, and `input[type="range"]`. Removes the iOS Safari grey/blue tap-highlight rectangle that broke the brand look, and removes the 300 ms double-tap-to-zoom delay that made the In Onda pill feel laggy. Pinch-zoom on the page itself is preserved.
- **Admin brand wordmark — golden "Mi" accent was uncoloured (same regression as #270)**: `<span class="mi">Mi</span>` in the admin sidebar `.a-brand` depended on the dead `.sidebar-station h1 .mi` rule whose ancestor selector never matched the new admin DOM. The active `.a-brand .wm em` rule only targeted `<em>`, not `<span class="mi">`. Restored by extending `.a-brand .wm em` to also match `.a-brand .wm .mi`. Also removed the orphan `.sidebar-station h1` block (7 lines of dead CSS).
- **Admin form fields triggered iOS Safari auto-zoom on focus**: `.search-input` and `.input` (search box, Anthropic key, OpenAI key) shipped at `font-size: 13px`. iOS Safari auto-zooms any focused form field below 16 px and knocks the layout sideways. Bumped to 16 px.
...
- **`/regia` route + `regia.html` template** — the Regia design language already shipped on `/admin` (admin panel title is "Mamma Mi Radio — Regia"), but the standalone `/regia` URL still served an obsolete prototype duplicate. Route handler in `mammamiradio/web/streamer.py`, the `_REGIA_HTML` constant, and the 1,499-line `mammamiradio/web/templates/regia.html` template are gone. Operators land on `/admin` for the control room.
...
- Standalone `Dockerfile` base image: `python:3.11-slim` → `python:3.14-slim`. The HA addon Dockerfile is separate (Alpine-based) and unaffected.
...
- **Listener page — Italian flag tricolor + radio cabinet rendered transparent**: PR #235 (Volare Refined) referenced `var(--flag-green)`, `var(--flag-red)`, `var(--flag-white)`, `var(--terracotta)`, `var(--terracotta-lt)`, `var(--sage)`, `var(--ink)` in listener.css and base.css but never declared the corresponding `:root` tokens. The browser silently rendered every Italian tricolor element (radio cabinet strip, hero stage seam, dedica form border, hero tricolor) and the entire vintage radio illustration body/knobs/antenna as `rgba(0,0,0,0)`. Now declared in `mammamiradio/static/tokens.css` with warm copper-brown cabinet (`#6B3E2D`) and tan highlights (`#B47850`) so the radio pops against the espresso page bg. Added `tests/test_design_tokens.py::test_every_var_ref_resolves_to_a_defined_token` to fail CI if any CSS file references a `var(--foo)` (no fallback) that isn't declared in tokens.css — closes the regression class.
- **Listener Dediche & Saluti — massive whitespace next to the form**: when only the placeholder dedica was visible, `.mmr-dediche-grid` had `align-items: start` so the small placeholder card sat at the top of its column while the much taller form (~301px) created a 150px void below the card. Switched to `align-items: stretch` and made `.mmr-dedica` a flex column with `justify-content: center`, so the placeholder card now matches the form height and centers its copy.
- **Listener "La stazione" cards barely visible**: `.mmr-about-card` used `var(--surface)` (#251E19) on a `--bg` (#14110F) page bg with a 10% cream border — the cards faded into the background. Bumped to `--surface-strong` (#362B25) + `--line-strong` (16% cream) so the four About cards are clearly defined against the page.
...
- **Producer wakes immediately on session resume**: replaced 1-second `asyncio.sleep` poll with `asyncio.wait_for(state.resume_event.wait(), timeout=1.0)`. `StationState` now carries a `resume_event` asyncio.Event; both the streaming gateway and `/api/resume` set it on session resume, collapsing the worst-case resume lag from 1s to milliseconds.
- **Silence fallback never queues a silent track**: audio quality circuit breaker now recycles the last-known-good music file (via `_get_last_music_file(state)`) or drops the segment rather than letting silent audio reach the playback queue. Prevents dead-air from corrupt/zero-length norm files reaching listeners.
- **LRU cache eviction respects playback queue**: `evict_cache_lru` now accepts an optional `protected_paths: set[Path]` argument. Producer passes currently-queued norm paths so in-flight audio is never deleted mid-stream.
- **LLM prompt injection hardening**: `_sanitize_prompt_data` now strips six quote variants (`"`, `` ` ``, `"`, `"`, `'`, `'`) and fake role markers (`System:`, `Assistant:`, `Human:`, `User:` — case-insensitive, with optional spacing). Prevents listener-submitted metadata from breaking out of interpolated prompt strings or injecting synthetic conversation turns.
...
- **Listener client migrated to public API**: `mammamiradio/static/listener.js` no longer polls admin-only `/status` + `/api/capabilities` + `/api/listener-requests`. It now uses `/public-status` for brand + capabilities + facts + `ha_moments` in one shape and `/public-listener-requests` for the dediche feed. Result: works on any deploy (loopback, LAN, public) without 401 risk. Wires `window.mmrApplyCaps(caps)` so `[data-cap=KEY]` elements toggle correctly, flips body `data-state` warming→live on first successful response, and removes the hardcoded "La sezione dediche si riempirà..." fallback that was overwriting the template empty-state copy.
- **Brand themes via CSS custom properties**: `mammamiradio/static/listener.css` migrated to use `var(--brand-primary, var(--sun))`, `var(--brand-accent, var(--lancia2))`, `var(--brand-bg, var(--shadow))` so listener tokens read from per-station `[brand.theme]` injected by `listener.html`, with Volare Refined as fallback. Each operator's station now visibly themes (color + font) without breaking the default dark-canvas foundation; contrast and font validation still guard against unreadable combinations.
- **OpenGraph social cards**: new `mammamiradio/og_card.py` module renders 1200×630 OG card PNGs via Pillow. Cards carry the Italian flag tricolor at the top edge, brand identity in an italic display font, an amber glow at top-left, a gold `ORA IN ONDA` eyebrow, and a lower-third track band. Honors per-station `[brand.theme]` colors. New `/og-card.png` route serves cached PNG (60s cache, evicts on track change) and falls back to the logo SVG on render failure so social previews never 404. `pillow>=10.0` added to dependencies.
- **Listener Jinja2 template + capability-conditional rendering**: `mammamiradio/listener.html` migrated from string-replace placeholder to a real Jinja2 template. Every brand-fiction string (station name, frequency, city, founded year, tagline, about, host display names) reads from the `[brand]` block at render time. New `--brand-*` CSS custom property namespace overlays Volare Refined defaults per station theme, with `var(--brand-primary, var(--sun))` fallback so `listener.css` works with or without a brand theme. Capability-conditional copy renders client-side via `[data-cap=KEY]` attributes: JS reads `/public-status.capabilities` every poll and toggles HA / AI / PWA copy based on actual capability flags. PWA install link (was: `<a href="/static/manifest.json">`) replaced with proper `<button>` + `beforeinstallprompt` flow. New Jinja2 dependency added. Empty-state copy stays in brand voice ("Aspettiamo la prima dedica della sera…", "Il palinsesto sta arrivando…", "Stiamo accendendo la radio…").
- **Public listener API (`/public-status` + `/public-listener-requests`)**: `/public-status` now returns a `brand` block (station_name, frequency, city, founded, tagline, hosts, theme tokens), a `capabilities` flag set (llm, anthropic_key, openai, ha, anthropic_degraded), `uptime_sec`, and `tracks_played` for cross-page parity. New `/public-listener-requests` endpoint serves a filtered dediche feed (drops internal IDs, raw timestamps, error fields). Cross-page invariant tests (`tests/test_public_status_contract.py`) keep admin and listener facts aligned.
- **Brand engine schema (`[brand]` block in `radio.toml`)**: separates brand-fiction layer (what listeners read) from operator-truth engine config (what operators control). New `BrandSection`, `BrandTheme`, `BrandHost` dataclasses in `config.py`. Theme tokens (primary/accent/background colors, display/body/mono fonts) override Volare Refined defaults per station, with constrained-variable guardrails: hex validation, background lightness ≤ 25 (dark-canvas invariant), 4.5:1 contrast against `--cream` body text, fonts from a curated allow-list of 6 (Playfair Display, Cormorant Garamond, Bodoni Moda, Lora, Outfit, JetBrains Mono for display; Outfit, Inter, Source Sans 3, IBM Plex Sans for body). `[[brand.hosts]]` maps `engine_host` (FK to `[[hosts]].name`) to a `display_name` and `description` so the listener can call them "Marco del bar" while the engine voices them via `Marco`. All validation is graceful: invalid values fall back to Volare defaults and surface as `brand_warnings` in admin `/status` for the Engine Room panel, and bad brand config never blocks station boot.
...
- **Regia admin prototype at `/regia`** (dev preview, admin-gated): Screen 1 ON AIR of the new admin UI. Persistent 16px status strip, 5-tab bar (only ON AIR wired; CODA/REVISIONE/PALINSESTO/MOTORE are placeholders), hero Now Playing with Playfair italic track name, countdown as Italian prose, banter preview as editorial pull-quote with gold quote glyph + Lancia red dropcap, 4-button trigger row (AVANTI / PAUSA / VOCE AI / SPOT), 260px read-only peek panel showing 8 upcoming items, ambient FM dial. Polls `/status` every 3s; AVANTI/VOCE AI/SPOT wired to existing `/api/skip` and `/api/trigger`. PAUSA and PANICO log warnings pending backend endpoints. `admin.html` untouched — this is a prototype at a new route, not a replacement.
...
UI truth and playback safety fixes — five admin and log fixes, plus a normalizer safety guard.

### Fixed

- **Normalizer concat duration guard**: `concat_files` now probes input durations with `ffprobe` and logs a `WARNING` when the concatenated output is shorter than the sum of its inputs, surfacing silent truncation instead of producing a mysteriously short track. Fail-open: when ffprobe is unavailable or cannot parse, the guard stays silent rather than blocking playback.
- **Stopped state actually stops**: Clicking Stop now freezes the dashboard animations, pauses the elapsed-time counter, and disables producer buttons until Resume. Previously the UI kept ticking as if the stream were live.
- **Three-state Anthropic status**: Admin panel distinguishes *connected*, *not configured*, and *suspended* (401 from Anthropic) instead of flashing "connected" while every script call was failing.
- **Scheduler reason strings no longer leak to listeners**: Up-next rows used to render raw strings like `"cooldown: 45s"` and `"banter_due_in=3"`. Those are now stripped before the row reaches the UI.
- **Raw norm-cache filenames never show as titles**: The rescue path that replays a pre-normalized track on producer stall no longer displays `Recovered: norm_abc123.mp3`. Sidecar metadata is used when present; otherwise the filename is humanized (`norm_busted.mp3` → `Busted`).
...
- Commit prefix docs now match the hook allowlist (added `merge:`).
...
- Expanded setup and runtime docs so setup, auth, fallback behavior, and runtime flow match the current code.
