...
- **The station can now keep a full rotation ready to play.** The normalization cache used to stop at 500 MB, roughly 100 songs. Standalone still defaults to 500 MB; the add-on now defaults to 1500 MB and exposes a **Music cache size (MB)** setting so you can match the cache to your rotation. A normalized song uses about 5 MB, and **On-Air Sound** roughly doubles that. A cache that is too small makes a few songs repeat while other tracks are rebuilt when they come around. At startup, the station checks free space and lowers the effective limit when needed, logging both values. If the disk is nearly full, the log tells you to free space.
- **The v1 integration contract is now a frozen, machine-checked artifact.** `CONTRACT.md` documents the frozen now-playing surface, the additive-only policy, and how a contract change lands. A golden fixture rendered by the real serializer is committed at `tests/integrations/golden/v1_now_playing.json`, and a contract-drift CI check verifies on every pull request that the serializer still produces those exact bytes, that frozen paths only change with an explicit `Contract-Change:` marker, and that the fixture matches its sibling copy in the Music Assistant provider once that copy exists.
- **A long listening stretch can now earn one honest companionship beat.** After 30 minutes of active listening in one anonymous station epoch, one naturally scheduled host break may acknowledge the shared listening moment without claiming that anyone just arrived, returned, or was identified. Empty-room time does not count, the cue is attempted at most once per epoch, and its coarse lifecycle appears only in authenticated diagnostics; `/public-status` is unchanged.
...
- **Station memory now follows the room, not individual connections.** The live stream hub remains the only connection-membership authority; gaps shorter than 10 minutes keep one in-memory station epoch, while a gap of 10 minutes or more starts another. Legacy active, peak, total, and connection counters remain available, and durable station-session updates use commit receipts so retries cannot double-count.
...
- **A curly apostrophe or an unusual letter in the station name no longer silences the station.** Naming your station something like `Let's Radio` on a Mac or iPhone quietly stores a typographic apostrophe instead of a straight one, and that single character was enough to make every attempt to play fail: the page loaded, the tuner spun, and no audio ever arrived. Names built from letters outside the Western European set did the same thing, so `Radio Łódź` and `Radio Čačak` stopped the stream dead too. Station names and themes now reach the player safely — `Radio Città` keeps its accent, curly quotes and long dashes become their plain equivalents, `Radio Łódź` reads as `Radio Lódz`, and anything a player still cannot carry is left out rather than taking the stream down with it. A name made up entirely of such characters falls back to the default station name instead of reaching the player blank.
...
- **Home Assistant Stop/Resume continuity is now restart-safe.** Stop persists before playback changes, Resume reserves readable recovery audio before unpausing, and both warm-cache and packaged-only startup paths are release-checked.
- **Home Assistant hot backups no longer omit the app when generated audio changes mid-copy.** The station keeps playing while Home Assistant skips temporary and rebuildable audio; those caches warm again after a restore. App settings, provider keys, station memory, and retained history remain in the backup. Because retained files are copied while playback continues, the result is not a copy taken from one single exact moment.
- **A host can no longer end up answering themselves mid-break.** When a written line was unusable — stage direction instead of speech, a repeated line, a guest cameo that was not invited — the station used to quietly drop it and air whatever was left. If the dropped line sat between the two hosts, their surrounding lines welded onto one speaker and a host appeared to reply to themselves; a break could also shrink to a single line and skip the length check that would have caught it. A shortened exchange now either still reads as a real conversation or gives way to the complete stock break, the hosts are asked up front not to write stage directions into spoken copy, and every dropped line is counted in the operator's Show Memory trail instead of vanishing.
- **Hosts now read your outdoor and sensor temperatures in the unit they were actually measured in.** A household running Home Assistant in Fahrenheit or Kelvin used to hear its readings announced as if they were Celsius, so 68 °F became "sixty-eight degrees" on air. Weather and classified temperature sensors are now converted before anything reaches the host, the news flash, or the control room, and the number is rounded to something a person would actually say instead of a long decimal. When Home Assistant will not say which unit a reading is in, the station names the conditions and leaves the number out rather than guessing — on the live weather line, in the forecast, and for sensors alike — then retries on the next poll instead of going a full hour without weather. Thermostats are the one exception and are not covered yet: Home Assistant does not publish a unit alongside a climate reading, so those are still read as Celsius.
- **Banning, moving, or queuing a rotation song now always acts on the song you clicked.** Control-room rotation edits used to trust the row's position, so if the playlist shifted underneath you — a source switch, a refresh, someone else editing — a Ban or Next could land on the wrong track. Each edit now carries the song's own identity and the rotation version it was shown against: if the list moved first, the station refuses the edit with a plain "the rotation changed, refresh and try again" instead of hitting the wrong song, and search results follow the same rule so a stale result can never redirect a click.
- **A failed queue rebuild now keeps the last safe audio playing.** Source switches, purges, Panic, Chaos, Festival, and pool clears retain the protected slot and first playable queued segment until replacement audio exists. Continuity audio is checked against current bans again when playback claims it, so a failed rebuild cannot open a gap or revive a newly banned track.
- **A failed voice can no longer turn into a silent spoken segment.** If every configured provider and fallback fails, partial audio is discarded and required speech falls through to approved canned copy or existing music/continuity recovery. News, IDs, sweepers, time checks, dialogue, and ads can no longer succeed as only a bed, sting, or chime.
- **Several smaller fixes keep the show and control room honest.** The listener now shows the configured stream bitrate, cut-off or unpaired host exchanges receive complete fallback copy, invalid or unreviewed ad voices fail closed, and a delayed control-room status request offers a safe retry instead of leaving the panel stale.
- **Failed or stale companionship copy can no longer sneak back into a later break.** A cue is claimed before writing, permanently abandoned by any generation, voice, quality, admission, purge, stop, overflow, or playback failure, and marked consumed only after a listener queue accepts audio. Playback drops an old-epoch cue before its next byte, while the final English/Italian truth guard repairs once and then uses deterministic safe copy. Packaged spoken recovery audio is now transcript-reviewed and hash-pinned; changed or unlisted welcome/banter clips fail closed.
...
- **Home Assistant context no longer competes with live stream timing.** Once the Home Assistant reply arrives, the heavy work of turning it into safe on-air context happens off the path that keeps audio moving. The final result still rechecks live mutes before use, so a muted entity cannot return later as a delayed host cue. If that work cannot run at all, the music carries on and only the house references pause until it can.
- **Blocked or failed music sources no longer become silent songs.** When YouTube or yt-dlp rejects a track, including an HTTP 403, the station marks it unavailable before probing or normalization and selects another candidate. If no accepted candidate is ready, its continuity-recovery ladder keeps the stream audible; rejected normalization files are removed so they cannot return as fallback audio.
...
- **Switching sources, purging, or flipping a mode can no longer open a gap or bring back a song you removed.** Every control that rebuilds the playback queue — source switches, playlist purges, panic, Chaos and Festival cutovers, and the everyday shuffle, add, move, and ban edits — now rebuilds the queue and its rundown in one step and reserves only audio that is already safe to play: eligible cached music first, then the branded continuity clip, then a packaged emergency tone as the cold-cache last resort. Songs already banned or removed when the reserve is rebuilt are excluded from that instant-audio reserve, and a track that was being prepared for the old lineup is discarded instead of arriving after the change. If the reserve bookkeeping ever hiccups, the show simply plays on.
- **A cached song can no longer air over and over while the station is catching up.** When live production stalls and the station bridges on already-normalized music, it now rotates through the cached tracks instead of re-picking at random. A song that just aired as a fill rests for an hour before it can be chosen again, and when every cached track is still resting the station airs the one heard longest ago — so a listener gets a real song they last heard twenty minutes ago instead of the same one three times in a row. Music always plays; the rotation never falls back to silence or a tone when a cached song is available. The control room gains an admin-only rescue-rotation readout so an operator can see it working.
- **The song playing right now can no longer be picked as the song that plays next.** Using a control mid-song — skip, ban, a mode toggle, an edit to the rotation — makes the station set aside a little safety audio first, and that audio could previously be the song already on the air. A song counted as heard only once it finished, so two minutes into a three-and-a-half-minute track it still looked untouched. It now counts as heard from the moment a listener receives it, and the safety audio is picked with the current song ruled out. The reservation a live control makes, and the first reach for cached music, both refuse to re-air the song on the air and drop to the next rung instead. The deeper rescue rungs prefer a song you have not just heard and re-serve a recent one only when the cache holds nothing else, because a song from a few minutes ago beats a four-second ident on a loop. The control room's Queue rescue row now reports these moments, which it never did before. Two controls in half an hour will not flip it to a warning.
- **A canned line no longer introduces music that was ready to play anyway.** When the station reaches for safety audio and a normalized song is sitting warm in the cache, it goes straight into the song. The short branded clip stays for a cold cache, where it is the difference between a voice and silence. It also reaches first audio sooner: the cached song already knows its own length, while the clip has to be measured first.
- **Adding songs to the rotation no longer throws away the song being prepared.** Growing, shuffling, or reordering the rotation used to discard whatever the station was in the middle of building — on a Raspberry Pi that is minutes of work binned, and it opened the very gap that then had to be covered with safety audio. Enabling Chaos Mode did this to itself, because Chaos adds its own tracks. Prepared audio is now only set aside when its song genuinely leaves the rotation, and host breaks, ads, and news are never affected by a rotation edit at all.
...
- **A host break or ad no longer airs silence if every voice provider fails.** When OpenAI, Azure, ElevenLabs, and Edge are all unavailable for a segment, the station skips it and falls back to a pre-recorded clip or its existing recovery audio instead of a synthesized silent clip. A render that gets cancelled mid-voice now waits for any in-progress audio work to actually finish before cleaning up its scratch files, closing a race that could otherwise delete audio still being written.
...
- **The station's "how much safe audio is ready" count can no longer be fooled by audio that can't actually play.** Queued or reserved segments that are banned, were queued for a listener who has since moved on, or are missing, empty, or unreadable on disk are no longer counted toward the cushion of ready audio that decides whether Skip has to cut straight to fresh music. A banned song sitting at the front of the queue is now discarded before playback reaches it, and removing queued items — by ban, by a skip, or by an operator's manual removal — can no longer leave a leftover, unverified segment mistakenly reused as backing music under the next host break.
- **Installing or updating the Home Assistant add-on works again.** The add-on repository was naming the next, unreleased version number, so Home Assistant offered an update whose image had never been published. Updates could not download, and fresh installs failed and rolled back. The repository now names only a published version, and a daily check raises a flag if that ever slips. Stations already playing were never interrupted, because Home Assistant downloads the new image before stopping the running one.
- **Station setup guidance no longer hides behind the live console.** On a desktop control room the setup strip was painted over by the pinned console's backdrop: where it should have read "Station setup — AI hosts: no key yet — Add AI key" there was only a blank gap, and the button, though invisible, still took keyboard focus and still worked. An operator could tab into a control they could not see, and the one prompt pointing a new station at its missing AI key was the thing being hidden. The strip now sits above the backdrop.
- **The control room works with a screen reader, on a phone, and in dim light.** Each host personality slider now says which host and which trait it belongs to, instead of presenting fifteen identically unnamed sliders. The Scaletta and Archivio filter chips report which one is currently selected rather than showing it through colour alone, and the AI Quality control announces its name. Faint helper text, empty-state notes and the pacing counters were too dim to read against the panel and have been lifted to a legible level, as has the red on destructive buttons — the panel keeps its espresso surface, only the writing on it changed. On phones the cadence sliders are now a full finger-height target instead of a hairline, while the track itself keeps its height.
- **The listener page reads clearly and is comfortable to tap.** The "In Onda" badge was set in the station's own accent red on a red ground, which left it barely legible; it now uses the station's cream on that red, as the rest of the site already did. The name field on a dedication, and the links in the footer, were smaller than a fingertip and have been brought up to a proper touch size.
- **The time counter can no longer run past the end of what is playing.** During a stretch of standby audio the elapsed time kept counting while the total stayed at four seconds, so the player showed things like "3:24 / 0:04". A second or two over is normal and is now smoothed; well past the end means the clock is no longer describing what you are hearing, and the player says nothing rather than something untrue.
