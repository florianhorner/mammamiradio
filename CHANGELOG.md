...
- **`mammamiradio/` subpackaged into seven domain naves**: 25 flat python files reshuffled into `core/` (config, models, capabilities, setup_status, sync), `audio/` (normalizer, audio_quality, tts, voice_catalog), `playlist/` (playlist, downloader, song_cues, track_rationale, track_rules), `hosts/` (scriptwriter, persona, context_cues, ad_creative), `home/` (ha_context, ha_enrichment), `scheduling/` (producer, scheduler, clip), and `web/` (streamer, og_card, plus templates/ and static/). HTML templates moved out of the package root into `web/templates/`. The whole `static/` tree moved to `web/static/`. `demo_assets/` and `logo.svg` moved into a new `assets/` tree. `tests/` mirrors the source tree exactly (61 test files into nine subpackage dirs). Used `git mv` everywhere to preserve history. Public addon entrypoint `mammamiradio.main:app` is unchanged. New `docs/REPO_MAP.md` answers "I want to fix X — where do I look?" in one table. README's first viewport rewritten to a 30-second pitch with three navigation links per leadership principle #4. `web/streamer.py` (~2,400 LOC) and `hosts/scriptwriter.py` (~1,500 LOC) carry `# TODO: split` markers; their actual splits are deferred to PRs 5 and 6 of the cathedral plan. Phase 3 of the cathedral restructure plan; closes the structural-failure finding from the 2026-04-28 DX review. **Migration note for any out-of-tree script that imports modules directly:** flat paths like `mammamiradio.config`, `mammamiradio.streamer`, `mammamiradio.playlist`, etc. no longer resolve; rewrite to nave-prefixed paths (`mammamiradio.core.config`, `mammamiradio.web.streamer`, `mammamiradio.playlist.playlist`, …) per `docs/REPO_MAP.md`. The HA addon entrypoint and Docker invocations are unaffected.
- **Repo root collapsed to four sacred files; everything else moved under `docs/`**. Phase 1 of the cathedral restructure plan; see `docs/2026-04-28-cathedral-restructure.md` for the full sequence. All moves used `git mv` to preserve history.

  **Sacred files kept at root** (one viewport, one job each):
  - `README.md`
  - `CONTRIBUTING.md`
  - `CLAUDE.md`
  - `CHANGELOG.md`
