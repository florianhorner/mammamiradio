...
- **Playlist serialization expanded** — `/api/status` `playlist` array now includes `album_art`, `source`, `year`, and `youtube_id` per track so the admin UI can render thumbnails and badges without extra round trips. Previously only `title`, `artist`, `display`, `spotify_id` were serialized.
