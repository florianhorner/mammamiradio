...
- **The station can now keep a whole rotation ready to play.** The music cache had a fixed 500 MB ceiling — around 100 songs, about half a typical rotation. The rest were thrown away before they came round again and had to be rebuilt from scratch, roughly a minute each time, while a listener waited. The add-on now keeps 1500 MB by default and exposes a **Music cache size (MB)** setting so it can be matched to the size of your rotation. If you have been hearing the same handful of songs while others never seem to play, this is why, and raising it fixes it. Turning On-Air Sound on roughly doubles what each song takes up, so give it more room if you use it. On startup the station checks the disk and lowers the ceiling if there is not enough room, telling you in the log when it does; if the disk is genuinely full it says so plainly rather than pretending a smaller number solved it.
