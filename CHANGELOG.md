...
- **Long-form YouTube results stay out of automatic rotation.** Direction searches, external adds, listener requests, and restart handoff now hold sets, full albums, podcasts, and oversized candidates before they can become the next song; single-track music still passes when it fits the station's current pacing.
...
- **Norm-cache rescue tracks now carry a real duration estimate.** Fresh normalized cache entries save track duration, older entries fall back to a bitrate-size estimate, and rescue/bridge segments can show progress without blocking instant audio on an `ffprobe` call.
