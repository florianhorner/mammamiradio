...
- **A curly apostrophe or an unusual letter in the station name no longer silences the station.** Type a name like `Let's Radio` on a Mac or iPhone and it stores a typographic apostrophe rather than a straight one. That one character was enough to make every attempt to play fail: the page loaded and the tuner spun, but no audio ever arrived. Names built from letters outside the Western European set did the same thing, so `Radio Łódź` and `Radio Čačak` stopped the stream dead too. Station names and themes now reach the player safely. `Radio Città` keeps its accent, curly quotes and long dashes become their plain equivalents, and `Radio Łódź` reads as `Radio Lódz`. Anything a player still cannot carry is left out rather than taking the stream down with it, and a name made up entirely of such characters falls back to the default station name instead of reaching the player blank.
