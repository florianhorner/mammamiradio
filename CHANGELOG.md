...
- **Your music now lives in Home Assistant's Media browser.** Add and remove songs through **Media → My media → mammamiradio** instead of copying files into the add-on data folder. The station scans that library every minute, keeps legacy `/data/music` readable on upgrade, and exposes **Scan now** in Rotazione for an immediate refresh.
