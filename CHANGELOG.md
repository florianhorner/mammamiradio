...
- **Your own music is the base again, and it airs under its real name.** On a Home Assistant Green with thirty songs in `/data/music`, the station was still playing the bundled starter catalog most of the time — about one local play in eight — because the scanner added the files without telling the station's song picker that anything had changed, and selection kept walking the starter bag in order. Untagged filenames also reached Now Playing and the hosts as `Unknown – <filename>`, with every local track timed as three and a half minutes. The scanner now reads title, artist, and duration from the file tags (falling back to the `Artist - Title` filename convention), and treats your own files as the station's base whenever they are there while still cycling the starter bag without early repeats. Reading those tags costs the station almost nothing: each file is read once, and again only when that file changes, and a read the station cannot finish leaves a song it already knows exactly as it was. If you run the Italian charts, your own music leads the rotation while the charts keep refreshing underneath it. The recovery bridge that inserts a starter when the queue is empty asks for starter media explicitly, so fixing the mix does not hand it a local track and fall through to a canned voice clip.
