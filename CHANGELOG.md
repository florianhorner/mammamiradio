...
- **Repository restructure plan**: a file-by-file doc maps the move from 27 flat files to 7 subpackages (`core`, `audio`, `playlist`, `hosts`, `home`, `scheduling`, `web`), mirrors the test tree, consolidates root docs under `docs/`, and rewrites README to a 30-second pitch. No code moves yet — this commit ships the plan only.
