...
- **The song playing right now can no longer be picked as the song that plays next.** Using a control mid-song — skip, ban, a mode toggle, an edit to the rotation — makes the station set aside a little safety audio first, and that audio could previously be the song already on the air. A song counted as heard only once it finished, so two minutes into a three-and-a-half-minute track it still looked untouched. It now counts as heard from the moment a listener receives it, and the safety audio is picked with the current song ruled out. Fallbacks that have other audio beneath them drop to the rung below rather than repeat. The one rung with nothing beneath it prefers a different song and repeats only when the cache holds nothing else, because a song from a few minutes ago beats a four-second ident on a loop. The control room's Queue rescue row now reports these moments, which it never did before. Two controls in half an hour will not flip it to a warning.
