...
- **The song playing right now can no longer be picked as the song that plays next.** Using a control mid-song — skip, ban, a mode toggle, an edit to the rotation — makes the station set aside a little safety audio first. That safety audio could previously be the song already on the air, because a song only counted as "heard" once it finished: two minutes into a three-and-a-half-minute track it still looked untouched. A cached song now counts as heard from the moment a listener actually receives it, and the safety audio is chosen with the current song explicitly ruled out. Every fallback that genuinely has other audio beneath it now refuses to re-air the current song and drops to the rung below instead; the one rung with nothing real underneath still prefers a different song and only repeats when the cache holds nothing else, because a song you heard a few minutes ago beats the same four-second station ident on a loop. The control room's Queue rescue row also reports these moments, which it never did before, so an operator can see the station leaning on safety audio instead of finding out from a listener. They are counted and listed, but on their own they do not flip the row to a warning: using two controls in half an hour is an operator doing their job, not a station in trouble, and a warning that cries wolf is worse than the silence it replaced.
- **A canned line no longer introduces music that was ready to play anyway.** When the station reaches for safety audio and a normalized song is sitting warm in the cache, it goes straight into the song. The short branded clip stays for a cold cache, where it is the difference between a voice and silence — but a real station does not announce that it is still on the air, it just plays. This also reaches first audio sooner: the cached song already knows its own length, while the clip has to be measured first.
