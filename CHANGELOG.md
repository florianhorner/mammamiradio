...
- **Your own music leads mixed rotations again.** When local and bundled starter tracks share a playlist, selection now uses the normal weighted picker and gives local tracks a `2×` lift instead of walking the starter bag first. A playlist containing only starter tracks still keeps its no-repeat bag order. Queue recovery explicitly selects starter media, so it cannot accidentally choose a local track.
