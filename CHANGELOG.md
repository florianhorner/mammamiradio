...
- **Listener controls now do exactly what they say.** Every play surface switches visibly and accessibly between Listen and Pause, a stopped station presents an honest paused status instead of a false Resume action, and dedication fields never start audio. Empty dedications explain what is missing, successful requests stay visible, failed requests preserve the listener's message, and stale browser data can no longer replace the station name served by the radio.
- **The producer desk stays truthful on phones and while paused.** The live console fits 320px screens with full-size controls, backstage work is labelled as building ahead rather than live, failed status polls show a reconnecting state instead of frozen production, and stopped sessions reject Air Next or Next-track actions until the operator presses Start.
- **Install and first-listen guidance now matches each package.** Home Assistant uses the current Apps path and its wired chart or Jamendo sources, Docker documents its chart-backed quickstart, source checkouts keep the repo-local MP3 path, and the Edge channel says when its deliberately cut image may trail `main`.
- **Recovery guidance now keeps the running add-on intact.** When a problem needs a code change, the Home Assistant guide now sends it through a reviewed branch, a CI-built image, and one planned add-on update instead of asking operators to edit live files, clear the cache, and restart repeatedly.
...
- **Steering the station toward a vibe is now something you can actually hear.** When an operator uses "Find records" to point the station at a mood or era, those records now recur often enough to notice — the pull automatically scales to how big the rotation is, instead of getting lost in a large pool the way it used to. Cooldowns, bans, and artist variety still apply, so the station leans into your direction without turning into the same handful of songs on repeat.
- **Reasoning-capable AI models now work as the creative voice.** Models that think before they answer (such as Claude Fable 5) return their reasoning ahead of the script; the station previously misread that shape, discarded the finished Anthropic script, and silently re-generated it through the OpenAI fallback on every host break. Script parsing now reads past the reasoning to the script itself, so a creative-model override can use a reasoning model normally.
- **Recovery no longer gets trapped in a repeating continuity clip.** If the queue runs thin after Resume, idle wake-up, or an active-playback drain, the station now plays the short branded recovery clip once and then moves to cached music runway when any normalized song is available, instead of looping the same host line while a fresh track renders. The recovery clip also starts on time even while the station's audio workers are busy, and its length is remembered so replays never wait on a re-measure.
- **Health checks no longer flag a station that is audibly bridging as silent.** The silence alarm behind `/healthz` and `/readyz` now requires that nothing is airing at all, not just that the queue is empty — so a fresh install covering its first track render with the packaged continuity clip stays healthy instead of inviting a watchdog restart mid-recovery. Genuinely dead air still trips the alarm after 30 seconds.
...
- **Resuming the station now updates the listener and admin status right away.** After tapping Resume, the public and admin status panels no longer keep showing the old stopped-state copy while the playback loop waits for the next segment.
