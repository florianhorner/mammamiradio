...
def test_get_client_returns_same_instance():
    ...
    from mammamiradio.hosts.scriptwriter import _get_client

    with (
        patch("mammamiradio.hosts.scriptwriter._anthropic_client", None),
        patch("mammamiradio.hosts.scriptwriter._anthropic_key", ""),
    ):
...
def test_get_system_prompt_caches_by_host_config():
    ...
    from mammamiradio.hosts.scriptwriter import _get_system_prompt

    config = load_config(TOML_PATH)

    # Reset cache
    with (
        patch("mammamiradio.hosts.scriptwriter._cached_prompt_key", ""),
        patch("mammamiradio.hosts.scriptwriter._cached_system_prompt", ""),
    ):
...
def test_get_system_prompt_rebuilds_when_hosts_change():
    ...
    with (
        ...
        with patch("mammamiradio.hosts.scriptwriter._cached_prompt_key", ""):
            prompt2 = _get_system_prompt(config)
