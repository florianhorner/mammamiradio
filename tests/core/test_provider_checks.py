...
async def test_provider_check_classifies_anthropic_auth_and_openai_success(monkeypatch):
    ...
    # Balanced probes distinct Terra creative and Luna fast models, followed by
    # the separately configured TTS model.
    assert seen_auth_headers == ["Bearer openai-secret"] * 3
