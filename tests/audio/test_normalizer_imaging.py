...
def test_transition_sting_music_to_time_check_uses_branded_motif(tmp_path, mock_run, caplog):
    out = tmp_path / "sting.mp3"
    caplog.set_level("WARNING", logger="mammamiradio.audio.normalizer")
    ...
    with (
        ...
        mock_bed.side_effect = lambda p, *_, **__: p.write_bytes(b"motif") or p
        mock_concat.return_value = out
        result = generate_transition_sting("music", "time_check", out)
...
def test_transition_sting_time_check_to_music_uses_bumper(tmp_path, mock_run, caplog):
    out = tmp_path / "sting.mp3"
    caplog.set_level("WARNING", logger="mammamiradio.audio.normalizer")
