...
def test_generate_sfx_cash_register_failure_uses_simple_fallback(mock_subprocess):
    out = Path("/tmp/sfx.mp3")
    with (
        patch("mammamiradio.audio.normalizer._generate_cash_register", side_effect=RuntimeError("ffmpeg broke")),
        patch("mammamiradio.audio.normalizer.generate_tone", return_value=out) as mock_tone,
    ):
...
def test_generate_bumper_jingle_falls_back_after_aevalsrc_failure():
    ...
    with patch(
        "mammamiradio.audio.normalizer._run_ffmpeg",
...
def test_generate_sweep_negative_start_freq_raises():
    from mammamiradio.audio.normalizer import generate_sweep
...
def test_generate_sfx_sweep_failure_uses_sweep_fallback(mock_subprocess):
    ...
    out = Path("/tmp/sfx.mp3")
    with (
        patch("mammamiradio.audio.normalizer._generate_whoosh", side_effect=RuntimeError("boom")),
        patch("mammamiradio.audio.normalizer.generate_sweep", return_value=out) as mock_sweep,
    ):
...
def test_generate_sfx_hotline_beep_failure_uses_tone_fallback(mock_subprocess):
    ...
    out = Path("/tmp/sfx.mp3")
    with (
        patch("mammamiradio.audio.normalizer.generate_tone", side_effect=[RuntimeError("boom"), out]),
    ):
    ...
    with (
        patch("mammamiradio.audio.normalizer._run_ffmpeg", side_effect=[RuntimeError("ffmpeg broke"), None]),
        patch("mammamiradio.audio.normalizer.generate_tone", return_value=out) as mock_tone,
    ):
...
def test_generate_brand_motif_cleans_up_on_exception(tmp_path, mock_subprocess):
    ...
    out = tmp_path / "motif.mp3"
    with (
        patch("mammamiradio.audio.normalizer.generate_sfx", side_effect=RuntimeError("ffmpeg died")),
...
from mammamiradio.audio.normalizer import generate_foley_loop  # noqa: E402
