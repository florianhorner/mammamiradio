...
def _write_clip_pair(clips_dir: Path, clip_id: str, *, mtime: float) -> tuple[Path, Path]:
    clips_dir.mkdir(parents=True, exist_ok=True)
    audio = clips_dir / f"{clip_id}.mp3"
    sidecar = clips_dir / f"{clip_id}.json"
    audio.write_bytes(f"audio-{clip_id}".encode())
    sidecar.write_text(json.dumps({"clip_id": clip_id}), encoding="utf-8")
    os.utime(audio, (mtime, mtime))
    os.utime(sidecar, (mtime, mtime))
    return audio, sidecar


def test_publish_clip_removes_expired_audio_and_matching_sidecar(tmp_path):
    clips_dir = tmp_path / "clips"
    now = time.time()
    expired_audio, expired_sidecar = _write_clip_pair(clips_dir, "expired", mtime=now - 48 * 3600)
    fresh_audio, fresh_sidecar = _write_clip_pair(clips_dir, "fresh", mtime=now)

    clip_id = publish_clip(
        b"new-audio",
        clips_dir,
        sidecar={"clip_label": "New"},
        max_saved=50,
        max_age_hours=24,
    )
    ...
    assert (clips_dir / f"{clip_id}.mp3").read_bytes() == b"new-audio"
    assert json.loads((clips_dir / f"{clip_id}.json").read_text(encoding="utf-8")) == {"clip_label": "New"}


def test_publish_clip_enforces_exact_bound_and_evicts_oldest_sidecar_pair(tmp_path):
    clips_dir = tmp_path / "clips"
    now = time.time()
    oldest_audio, oldest_sidecar = _write_clip_pair(clips_dir, "oldest", mtime=now - 20)
    newest_audio, newest_sidecar = _write_clip_pair(clips_dir, "newest", mtime=now - 10)

    clip_id = publish_clip(
        b"replacement",
        clips_dir,
        sidecar={"clip_label": "Replacement"},
        max_saved=2,
    )

    assert not oldest_audio.exists()
    assert not oldest_sidecar.exists()
    assert newest_audio.exists()
    assert newest_sidecar.exists()
    assert sorted(path.name for path in clips_dir.glob("*.mp3")) == [f"{clip_id}.mp3", "newest.mp3"]
    assert sorted(path.name for path in clips_dir.glob("*.json")) == [f"{clip_id}.json", "newest.json"]


def test_publish_clip_replaces_sidecar_before_audio(tmp_path):
    clips_dir = tmp_path / "clips"
    ...
    with patch("mammamiradio.scheduling.clip.os.replace", side_effect=_record_replace):
        clip_id = publish_clip(b"audio", clips_dir, sidecar={"clip_label": "Named"}, max_saved=2)

    assert [destination for _source, destination in replacements] == [f"{clip_id}.json", f"{clip_id}.mp3"]
    assert replacements[0][0].endswith(".json.part")
    assert replacements[1][0].endswith(".mp3.part")


def test_publish_clip_serializer_failure_leaves_existing_archive_untouched(tmp_path):
    clips_dir = tmp_path / "clips"
    existing_audio, existing_sidecar = _write_clip_pair(clips_dir, "existing", mtime=time.time())
    ...
    with pytest.raises(TypeError):
        publish_clip(
            b"audio",
            clips_dir,
            sidecar={"bad": Unserializable()},
            max_saved=1,
        )

    assert existing_audio.exists()
    assert existing_sidecar.exists()
    assert sorted(path.name for path in clips_dir.iterdir()) == ["existing.json", "existing.mp3"]


def test_publish_clip_write_failure_removes_all_staged_and_final_files(tmp_path):
    clips_dir = tmp_path / "clips"

    with (
        patch("mammamiradio.scheduling.clip.os.fsync", side_effect=OSError("disk full")),
        pytest.raises(OSError, match="disk full"),
    ):
        publish_clip(b"audio", clips_dir, sidecar={"clip_label": "Named"}, max_saved=2)

    assert list(clips_dir.iterdir()) == []


def test_publish_clip_write_failure_preserves_archive_at_capacity(tmp_path):
    clips_dir = tmp_path / "clips"
    existing_audio, existing_sidecar = _write_clip_pair(clips_dir, "existing", mtime=time.time())
    ...
    with (
        ...
        publish_clip(b"audio", clips_dir, sidecar={"clip_label": "Named"}, max_saved=1)

    assert existing_audio.read_bytes() == b"audio-existing"
    assert json.loads(existing_sidecar.read_text(encoding="utf-8")) == {"clip_id": "existing"}
    assert sorted(path.name for path in clips_dir.iterdir()) == ["existing.json", "existing.mp3"]


def test_publish_clip_second_replace_failure_rolls_back_published_sidecar(tmp_path):
    clips_dir = tmp_path / "clips"
    real_replace = os.replace
    replace_count = 0

    def _fail_audio_replace(source, destination):
        nonlocal replace_count
        replace_count += 1
        if replace_count == 2:
            raise OSError("audio publish failed")
        real_replace(source, destination)

    with (
        patch("mammamiradio.scheduling.clip.os.replace", side_effect=_fail_audio_replace),
        pytest.raises(OSError, match="audio publish failed"),
    ):
        publish_clip(b"audio", clips_dir, sidecar={"clip_label": "Named"}, max_saved=2)
...
def test_publish_clip_replace_failure_preserves_archive_at_capacity(tmp_path):
    clips_dir = tmp_path / "clips"
    existing_audio, existing_sidecar = _write_clip_pair(clips_dir, "existing", mtime=time.time())

    with (
        patch("mammamiradio.scheduling.clip.os.replace", side_effect=OSError("publish failed")),
...
def test_publish_clip_eviction_failure_keeps_new_publication(tmp_path, caplog):
    clips_dir = tmp_path / "clips"
    existing_audio, existing_sidecar = _write_clip_pair(clips_dir, "existing", mtime=time.time())
    real_unlink = Path.unlink

    def _fail_existing_audio(path, *args, **kwargs):
        if path == existing_audio:
            raise OSError("archive busy")
        return real_unlink(path, *args, **kwargs)

    with patch.object(Path, "unlink", _fail_existing_audio):
        clip_id = publish_clip(b"new", clips_dir, sidecar={"clip_label": "New"}, max_saved=1)

    assert existing_audio.exists()
    assert existing_sidecar.exists()
    assert (clips_dir / f"{clip_id}.mp3").read_bytes() == b"new"
    assert (clips_dir / f"{clip_id}.json").exists()
    assert "clip capacity cleanup failed for existing.mp3" in caplog.text


def test_publish_clip_refuses_the_keepsakes_directory(tmp_path):
    keepsakes_dir = tmp_path / KEEPSAKES_DIRNAME

    with pytest.raises(ValueError, match="refusing to publish an expiring clip as a keepsake"):
        publish_clip(b"audio", keepsakes_dir, sidecar={"clip_label": "Never"}, max_saved=2)

    assert not keepsakes_dir.exists()


@pytest.mark.parametrize(
    ("clip_data", "sidecar", "max_saved", "message"),
    [
        (b"", {}, 2, "clip audio must be non-empty bytes"),
        (bytearray(b"audio"), {}, 2, "clip audio must be non-empty bytes"),
        (b"audio", [], 2, "clip sidecar must be an object"),
        (b"audio", {}, "2", "max_saved must be positive"),
        (b"audio", {}, 0, "max_saved must be positive"),
    ],
)
def test_publish_clip_rejects_invalid_archive_inputs(tmp_path, clip_data, sidecar, max_saved, message):
    clips_dir = tmp_path / "clips"

    with pytest.raises(ValueError, match=message):
        publish_clip(clip_data, clips_dir, sidecar=sidecar, max_saved=max_saved)

    assert not clips_dir.exists()


def test_publish_clip_does_not_evict_an_entry_with_unreadable_mtime(tmp_path):
    clips_dir = tmp_path / "clips"
    now = time.time()
    readable_audio, readable_sidecar = _write_clip_pair(clips_dir, "readable", mtime=now - 20)
    unreadable_audio, unreadable_sidecar = _write_clip_pair(clips_dir, "unreadable", mtime=now - 10)
    real_stat = Path.stat

    def _raise_for_unreadable(path, *, follow_symlinks=True):
        if path == unreadable_audio:
            raise OSError("mtime unavailable")
        return real_stat(path, follow_symlinks=follow_symlinks)

    with patch.object(Path, "stat", _raise_for_unreadable):
        clip_id = publish_clip(b"audio", clips_dir, sidecar={"clip_label": "New"}, max_saved=2)

    assert not readable_audio.exists()
    assert not readable_sidecar.exists()
    assert unreadable_audio.exists()
    assert unreadable_sidecar.exists()
    assert (clips_dir / f"{clip_id}.mp3").exists()


def test_publish_clip_second_staging_file_failure_closes_and_removes_first(tmp_path):
    clips_dir = tmp_path / "clips"
    real_mkstemp = __import__("tempfile").mkstemp
    calls = 0

    def _fail_second_mkstemp(*args, **kwargs):
        nonlocal calls
        calls += 1
        if calls == 2:
    ...
    with (
        patch("mammamiradio.scheduling.clip.tempfile.mkstemp", side_effect=_fail_second_mkstemp),
        pytest.raises(OSError, match="sidecar staging failed"),
    ):
        publish_clip(b"audio", clips_dir, sidecar={"clip_label": "New"}, max_saved=2)

    assert calls == 2
    assert list(clips_dir.iterdir()) == []


def test_publish_clip_fdopen_failure_closes_both_staging_descriptors(tmp_path):
    clips_dir = tmp_path / "clips"
    real_close = os.close

    with (
        patch("mammamiradio.scheduling.clip.os.fdopen", side_effect=OSError("fdopen failed")),
        patch("mammamiradio.scheduling.clip.os.close", wraps=real_close) as close,
...
def test_prune_stale_clip_tmp_files_removes_only_old_publication_scratch(tmp_path):
    clips_dir = tmp_path / "clips"
    clips_dir.mkdir()
    stale_audio = clips_dir / ".old-a.mp3.part"
    stale_sidecar = clips_dir / ".old-b.json.part"
    fresh_audio = clips_dir / ".fresh.mp3.part"
    public_audio = clips_dir / "public.mp3"
    unrelated = clips_dir / ".decoder.mp3.part.extra"
    orphan_sidecar = clips_dir / "orphan.json"
    fresh_orphan_sidecar = clips_dir / "fresh-orphan.json"
    public_sidecar = clips_dir / "public.json"
    for path in (
        stale_audio,
        stale_sidecar,
        fresh_audio,
    ...
    os.utime(orphan_sidecar, (old_time, old_time))

    removed = prune_stale_clip_tmp_files(clips_dir, max_age_hours=6)
    ...
    assert fresh_audio.exists()
    assert fresh_orphan_sidecar.exists()
    assert public_audio.exists()
