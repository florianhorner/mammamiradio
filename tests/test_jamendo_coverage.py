...
def test_validate_direct_url_accepts_jamendo_cdn():
    ...
    from mammamiradio.downloader import _validate_direct_url
...
def test_block_redirect_handler_raises_on_redirect():
    ...
    from mammamiradio.downloader import _BlockRedirectHandler
...
def test_download_direct_url_success(tmp_path):
    ...
    from mammamiradio.downloader import _download_direct_url
...
def test_download_direct_url_validation_failure_removes_file(tmp_path):
    ...
    with (
        patch("mammamiradio.downloader._NO_REDIRECT_OPENER") as mock_opener,
        patch("mammamiradio.downloader.validate_download", return_value=(False, "duration too short")),
...
def test_find_demo_asset_cache_dir_mismatch_forces_reglob(tmp_path):
    ...
    with patch("mammamiradio.downloader._DEMO_ASSETS_DIR", new_dir):
...
def test_jamendo_tags_reads_from_persisted_url(config):
    ...
    from mammamiradio.playlist import _jamendo_tags
...
def test_fetch_jamendo_playlist_network_error_returns_empty(config):
    ...
    config.playlist.jamendo_client_id = "cid123"
    with patch("mammamiradio.playlist.urlopen", side_effect=URLError("timeout")):
...
def test_load_explicit_source_jamendo_url_kind_resolves(config):
    ...
    from mammamiradio.playlist import load_explicit_source
...
def test_load_explicit_source_jamendo_empty_raises(config):
    ...
    from mammamiradio.playlist import ExplicitSourceError, load_explicit_source
...
def test_fetch_startup_playlist_persisted_jamendo_fails_falls_back_to_demo(config):
    ...
    from mammamiradio.playlist import ExplicitSourceError, fetch_startup_playlist

    config.allow_ytdlp = False

    with (
        patch(
            "mammamiradio.playlist.load_explicit_source",
...
def test_fetch_startup_playlist_jamendo_used_as_startup_source(config):
    ...
    from mammamiradio.playlist import fetch_startup_playlist
...
def test_fetch_startup_playlist_local_music_warning_when_ytdlp_disabled(config, tmp_path):
    ...
    with (
        patch("mammamiradio.playlist._load_demo_asset_tracks", return_value=[]),
        patch("mammamiradio.playlist.Path") as mock_path_cls,
    ):
