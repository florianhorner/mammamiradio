...
from mammamiradio.web.og_card import OGCardInputs, render_og_card
...
def test_render_og_card_for_brand_helper():
    ...
    from mammamiradio.core.config import BrandSection, BrandTheme
...
async def test_og_card_route_fallback_on_render_failure(monkeypatch):
    ...
    monkeypatch.setattr("mammamiradio.web.og_card.render_og_card_for_brand", boom)
    transport = httpx.ASGITransport(app=app, client=("127.0.0.1", 12345))
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        resp = await client.get("/og-card.png")
