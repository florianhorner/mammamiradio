...
- `MAMMAMIRADIO_HA_CONTEXT_REFRESH_TIMEOUT`: wall-clock budget (seconds, positive float) the producer gives a single HA context refresh before it airs on last-known (then empty) home context instead of blocking segment production. Maps to `[homeassistant] context_refresh_timeout` in `radio.toml` (env > toml; default `2.0`); a non-float or ≤0 value is ignored with a warning. Audio continuity wins over HA freshness (INSTANT AUDIO) — a slow/hung HA degrades context rather than stalling the loop. The one-time cold registry/weather warm-up gets a longer budget (`_HA_CONTEXT_COLD_LOAD_TIMEOUT` in `scheduling/producer.py`) so the device-label catalog can populate; every steady-state refresh gets this tight budget. Not exposed as an HA add-on UI option. `BANTER`/`AD`/`NEWS_FLASH` all refresh through the one budgeted helper (`_refresh_home_context_budgeted`).
...
- `MAMMAMIRADIO_SUPER_ITALIAN`: station personality dial (`true`/`1`/`yes` to enable; default off). When OFF (default), listener UI uses English utility copy with Italian headlines and station-feel words, and AI hosts speak roughly 70% English / 30% Italian (English carries the information; Italian carries greetings, exclamations, punchlines, and about one line in three may be a complete Italian sentence); admin stays English-first. When ON, listener UI is Italian and hosts speak 100% Italian (only song/artist/brand names and the guest host's Bavarian phraselets are exempt). The language policy is centralized in `hosts/prompt_world.py` (`language_mode_directive` for the banter system prompt, `language_mode_rule` for the news/ad/transition RULES lists and course-change notices), so every LLM speech surface follows the mode — news and ads no longer stay all-Italian inside an English-led normal-mode show. Operator-toggleable from admin Engine Room (hot-reloadable; persisted to `.env` in standalone mode and `/data/options.json` in HA addon mode).
...
- If Home Assistant is enabled, `HA_TOKEN` is present, and prompt-safe context has populated, banter and ads may reference current home state.
