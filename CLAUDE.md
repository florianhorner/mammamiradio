...
- Offline-first: operator-owned local `music/` files remain the base when present; otherwise the app uses the hash-pinned, attributed 12-track starter catalog directly. A release is blocked unless the exact files, at least 45 minutes, complete human audition evidence, and the 75 MiB package ceiling pass `make media-check` and `make media-proof`.
...
- Any chart refresh, external search/download commit, classic-era import, or external listener-request path described below is a standalone-only capability behind the effective `external-media` gate. In either Home Assistant add-on those operations fail with the locked actionable `403` envelope, local search still works, and a listener song request is retained only as a shout-out.
