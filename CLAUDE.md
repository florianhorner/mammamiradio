...
- `MAMMAMIRADIO_MAX_CACHE_MB`: ceiling for the normalization cache, in MB. Default `500` standalone, `1500` in the HA addon (`ADDON_MAX_CACHE_SIZE_MB` in `core/config.py`, which is also the addon option default). Clamped to `200`-`8000` by `_env_clamped_int`; a malformed or out-of-range value degrades to the clamp or the default with a warning rather than failing config load (a raised `ValueError` here means the station never boots — INSTANT AUDIO). A normalized track is ~5 MB, so the addon default holds a full ~200-track rotation; at the old 500 MB ceiling more than half a rotation stayed permanently cold and re-paid the ~65s cold render on every play. Enabling On-Air Sound roughly doubles per-track footprint (a `norm_` plus an `fm_` bake). At startup `_disk_safe_cache_ceiling_mb` in `main.py` trims the effective ceiling to what the disk can actually hold (free space + reclaimable cache − 512 MB reserve, never below the 200 MB floor) and logs a warm warning — a ceiling above free space never evicts, so the cache would grow until `/data` fills. Best-effort: an unreadable mount leaves the configured value alone. The HA add-on exposes it as the **Music cache size (MB)** option (`norm_cache_mb`, mapped in `run.sh`); applied at the next add-on restart, not a live admin toggle.
