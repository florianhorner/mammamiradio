...
- **Listener-request station identity is candidate-backed and request-independent.** Before a verified search result becomes a `Track`, artist-field feature credits and title credits that are bracketed or terminal separator-led suffixes collapse to one base artist/title identity: `Lady Gaga feat. Bradley Cooper – Shallow` and `Lady Gaga – Shallow (feat. Bradley Cooper)` both enter rotation as `Lady Gaga – Shallow`. Standalone title `feat.`/`ft.`/`featuring` text and compound separator tails remain literal because they are ambiguous, and a guest tail remains whole so band-name punctuation is not guessed into separate artists. The full candidate credit and verified guest identity remain admission-time blocklist aliases; listener spelling never supplies station or policy identity.
