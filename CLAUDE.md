...
- `docs/listener-qs-train.md` - `Train/Listener QS` intake, merge gate, and handoff contract (Path B only)
...
- `OPENAI_API_KEY`: OpenAI voice synthesis through the TTS model configured in `model_registry.toml`; also enables script generation fallback through the active quality profile when Anthropic is unavailable
...
- **OpenAI fallback script evaluator:** `scripts/eval_openai_script_model.py` is an online,
  ...
  Anthropic-first live output, or runtime enforcement. Run `--dry-run` first to validate the
  prompt corpus and see calls, completion-token bounds, and registry pricing without network
  activity. A real run emits schema-versioned JSONL receipts under `tmp/evals/`: parsed
  outputs carry deterministic raw-output integrity states for station name, banter roster,
...
- `JAMENDO_CLIENT_ID`, `MAMMAMIRADIO_JAMENDO_ENABLED`, `MAMMAMIRADIO_JAMENDO_NONCOMMERCIAL_ACKNOWLEDGED`, `MAMMAMIRADIO_JAMENDO_ACK_REVISION`: private facts for the explicit, default-off transient Jamendo source. Enablement requires the current non-commercial-use acknowledgement; a client ID is not part of the gate. The station ships its registered application ID as `BUNDLED_JAMENDO_CLIENT_ID` in `core/config.py`. Jamendo issues one client ID per application, not per listener. `JAMENDO_CLIENT_ID` overrides the bundled value, and malformed overrides fall back to the bundled ID with a warning. `playlist.jamendo_client_id_source` records the resolved credential source (`operator`, `bundled`, or empty). An empty `BUNDLED_JAMENDO_CLIENT_ID` restores operator-only behavior. Since an ID always resolves when bundled access exists, `capabilities.jamendo`, `playlist.py` readiness, and `status_payload.py` source projection use `jamendo_enabled`, not ID presence. Jamendo provider code 6 maps to `rate_limited`; the operator card reports automatic retry with no credential change presented as a remedy. The provider owns at most one lease and one single-use artifact. Neither may enter cache, SQLite, rescue, handoff, clips, derivatives, or restart state. **`JAMENDO_CLIENT_ID` remains separately required by `scripts/starter-catalog.py acquire` for a `provider: jamendo` row.** That bundled starter path is separate from the transient runtime source and uses attribution-only tracks. Jamendo's public track page publishes no licence, so the API is the authority and acquisition needs a free client ID; Incompetech does not. The API response is hashed into the candidate receipt, `check` stays offline, and the recorded URL has its credential stripped. Incompetech hashes two independent documents, the piece page and `pieces.json`; Jamendo has one API response, so `piece_sha256` and `pieces_sha256` contain the same digest.
...
- `MAMMAMIRADIO_SUPER_ITALIAN`: station personality dial (`true`/`1`/`yes` to enable; default off). When OFF (default), listener UI uses English utility copy with Italian headlines and station-feel words, and AI hosts target roughly 75% English / 25% Italian (English carries the information; Italian carries greetings, exclamations, punchlines, and occasional complete Italian sentences); admin stays English-first. When ON, listener UI is Italian and hosts speak 100% Italian (only song/artist/brand names and the guest host's Bavarian phraselets are exempt). The language policy is centralized in `hosts/language_policy.py` and consumed by `hosts/prompt_world.py`, so every LLM speech surface follows the mode — news and ads no longer stay all-Italian inside an English-led normal-mode show. Operator-toggleable from admin Engine Room (hot-reloadable; persisted to `.env` in standalone mode and Supervisor's stored `super_italian_mode` option in HA add-on mode).
- `MAMMAMIRADIO_FESTIVAL_MODE`: enable Festival Mode (`true`/`1`/`yes`; default off). Hosts become theatrical music competition MCs — fictional Italian-regional delegations, dramatic scoring, drinking game triggers. Toggleable live from the admin panel without a restart; persisted to `.env` in standalone mode and Supervisor's stored `festival_mode` option in HA add-on mode.
...
- Offline- and First-Listen-first: operator-owned local `music/` files remain the
  base when present; otherwise the app uses the hash-pinned, attributed 12-track
...
- If Anthropic fails mid-session, script generation falls back to the same role's
  OpenAI catalog entry from `model_registry.toml` through the active quality
  profile when `OPENAI_API_KEY` is set, then to short stock copy.
...
- Source switching via `/api/playlist/load` cuts over immediately only when fresh
  ...
  and the response reports `skipped: false`; the source revision still rejects
  in-flight renders from the prior source. If that assetless fallback preserved
  the exact queued listener dedication whose requested song the switch revokes,
  remove the dedication before swapping sources so the station cannot air a
  promise it no longer owns. A dedication already on air owns its exact song
  whether that song is admitted or still rendering: keep ready admitted music at
  the queue head, or restore a handoff with fresh force/pin revisions when its
  admitted file vanished or its old render is fenced stale. In the retry case,
  hold fresh or preserved fallback audio in the capacity-exempt slot so the retry
  can queue first; the slot/playback recovery ladder covers a late retry. The
  dedication finishes and the response remains `skipped: false`.
- **Metadata-only ingest commits.** Slow playlist/source, heading, enrich, and
  external-download routes capture `continuity_epoch` before their unbounded work
  and re-check it at the commit boundary. `/api/direction` is the deliberate
  exception: its expansion is only a proposal until the serialized heading commit,
  so it captures the epoch inside that commit after rejecting a stale heading
  revision. When the station is stopped, or a pre-captured epoch moved while a
  request was in flight (a Stop, or a Stop plus a fast Resume), the route still
  applies the playlist/track/heading change but skips runway reservation entirely
  and returns `metadata_only: true` — metadata moves, no audio is admitted, nothing
  is skipped. A stale source load owns the crate, not the controls: it commits
  ...
  when the reason was a paused station, which is the narrower half of
  `metadata_only` (a mid-flight epoch move sets `metadata_only` without it); the
  external/listener download commit reports plain status strings instead. The
  admin reads the flag to say the change is saved but nothing new airs until
  Start, so an operator is never told a deferred change landed on air.
- **Listener-request station identity is candidate-backed and request-independent.** Before a verified search result becomes a `Track`, artist-field feature credits and title credits that are bracketed, terminal separator-led, or conservative punctuated abbreviations collapse to one base artist/title identity: `Lady Gaga feat. Bradley Cooper – Shallow`, `Lady Gaga – Shallow (feat. Bradley Cooper)`, and `Lady Gaga – Shallow ft. Bradley Cooper` all enter rotation as `Lady Gaga – Shallow`. Ambiguous literal or unpunctuated `feat`/`ft`/`featuring` wording remains part of the title unless a separately named guest corroborates the candidate's lowercase credit interpretation; compound separator tails remain literal. A guest tail remains whole so band-name punctuation is not guessed into separate artists. The full candidate credit and verified guest identity remain admission-time blocklist aliases; listener spelling never supplies station or policy identity.
- Any chart refresh, external search/download commit, classic-era import, or
  external listener-request path described below is a standalone-only capability
  behind the effective `external-media` gate. In either Home Assistant add-on
  those operations fail with the locked actionable `403` envelope, local search
  still works, and a listener song request is retained only as a shout-out.
- **Keepsakes (durable "Keep this").** Clips expire after 24h (`CLIP_TTL_SECONDS`) and the provenance ledger prunes after `MAMMAMIRADIO_LEDGER_RETENTION_DAYS` (default 14), so a moment worth keeping is destroyed twice over by default. `POST /api/clip/keep` (admin auth, no body) is the on-air console's **Keep this** button: it exports the voice segment currently on air, or within `CLIP_LOOKBACK_SECONDS` the one that just ended, to `cache_dir/keepsakes/` with a sidecar (`station_name`, `track_title`, `track_artist`, `segment_type`, `source`, `created_at`). Durability is structural, not a flag: `cleanup_old_clips` and the `CLIP_MAX_SAVED` cap operate on `clips/` alone, and every cache pruner globs `cache_dir/*.mp3` without recursing, so nothing that prunes the cache can see the directory; `cleanup_old_clips` additionally refuses outright when handed a directory named `keepsakes`, and `test_cache_eviction_never_reaches_into_keepsakes` guards the non-recursive assumption the other modules provide. **Provenance and byte ownership share one boundary.** The playback loop writes `app.state.clip_segment` (`{type, chunks, title, has_music_tail}`) at every segment boundary and increments `chunks` on each chunk that segment puts into the share ring; the route cuts exactly those chunks (`extract_segment_audio` in `scheduling/clip.py`) instead of deriving a byte length from wall-clock elapsed. The ring holds every voice segment back to back — music never enters it — so a window one chunk too long is the previous segment's audio wearing this one's title and rights check. The same count sizes the `last_shareworthy_clip` lookback snapshot, which is now written for all six keepable types rather than `AD`/`BANTER` only, since the console offers the button for all six. `clip_segment` is cleared on stop. **Two independent eligibility gates, both failing closed** (`is_keepsake_eligible`): the segment type must be in `KEEPSAKE_SEGMENT_TYPES` (banter, ad, news_flash, station_id, sweeper, time_check), **and** the segment must not carry `has_music_tail` — `commit_music_handoff` crossfades the outgoing song's real master under the opening seconds of the next voice segment, so type alone stopped being proof of provenance. A keepsake never expires and is served unauthenticated, so a tailed segment is refused rather than trimmed. The snapshot carries the flag too and its own recorded type is re-checked, not trusted. The lookback is consulted **only when the airing segment is not a keepable type**: `too_early` and `music_tail` are refusals about what is on air now, and falling through to the previous segment would have returned 200 and permanently saved something the operator was not listening to. Refusals name the actual situation (`music`, `music_tail`, `not_on_air`, `too_early`, `not_keepable`, `archive_full`, `no_room`, `write_failed`) with a concrete next step, never a catch-all. Anything shorter than `KEEPSAKE_MIN_SECONDS` of actual audio is refused as `too_early`. Bounded by `KEEPSAKE_MAX_SAVED` and a `KEEPSAKE_MIN_FREE_MB` free-space preflight, because this is the one cache directory nothing reclaims; counting the shelf, checking free space and publishing the file run under `_keepsake_write_lock` so two presses cannot both pass at the ceiling. The ring buffer is copied on the event loop and joined in a thread (`_snapshot_segment`), and the file write is also threaded, so a ~4 MB export cannot stall the playback sender against its 4s cushion; the write is `mkstemp`+`fsync`+`os.replace` so a kill mid-write can never publish a truncated keepsake in a directory with no evictor. The leftover `.keepsake-*.tmp` scratch is never listed, served, or counted toward the ceiling, is in the add-on's `backup_exclude` via `*.tmp`, and is swept at the next startup by `prune_stale_keepsake_tmp_files` (6h age gate, so a write in flight is never the thing pruned). **Revocation.** `GET /api/clip/keep` lists what is on the shelf and `DELETE /api/clip/keep/{id}` removes the audio and its sidecar (admin auth, `operator_action` row `unkeep_this`), surfaced as **Kept moments** in the Archivio tab plus an Undo on the keep toast. Deleting the file is the whole revocation, since both public routes read from disk per request. Keepsakes are served through the existing `/clips/{id}.mp3` and `/clips/{id}` URLs (checked **after** `clips/`, so an id can never resolve to a keepsake while a live clip of that id exists) and skip the TTL check; the console builds the copied link through the ingress-aware `_base` prefix and only reports "copied" once the clipboard write resolves. Each keep records an `operator_action` ledger row when the ledger is enabled. `POST /api/clip` (listener-facing) shares only complete bundled starter tracks since the copyright-safe sources work, so **Keep this is the only path by which host or ad audio leaves the station.**
- **Operator song blocklist (durable ban).** Removing a song from the rotation pool is a permanent ban, not an in-memory splice: the always-visible per-row "✕ Ban" button (`/api/playlist/remove`, with a by-key Undo toast) and the bulk "Ban selected" both write to `cache_dir/blocklist.json`. Rows retain the stored `normalized_track_key` `(artist.strip().lower(), title.strip().lower())`, while every hard gate uses `song_identity_key_is_blocklisted` so accents, intra-word punctuation, compact artist spacing, and conventional primary-artist feature credits cannot bypass the ban. The shared check covers startup/source refresh, external and listener downloads, restart handoff, producer admission, continuity and norm-cache rescue, and playback's final queue claim. The external/listener commit returns a distinct `"banned"` status so the admin gets an honest notice and a listener request fails loudly instead of hanging on "searching…". Bulk `/enrich` honors the blocklist; only an explicit single `/api/playlist/add` bypasses it (intentional override). Banning also clears a matching `pinned_track`, purges any not-yet-started queued segment of the song (the current segment finishes — no dead air), and — after that purge — revokes any leftover `listener_request_admitted_reservations` entry for the banned identity, which is how a promise already claimed by playback (no longer in the queue, so the purge cannot reach it) is stopped from re-arming itself as a retry on a pre-first-byte failure. The **one** exception is the on-air console's **Ban** button (`POST /api/track/ban-now-playing`): it bans and skips the airing song through `_request_skip`. A bulk ban that would starve the rotation is refused; a single per-row removal stays exempt. Persistence is best-effort and corrupt-tolerant, and the API reports `persisted: false` honestly when a session-only ban cannot be saved. Two **local**-file doorways — `filter_blocklisted` in `playlist/playlist.py` and `reconcile_local_library` — additionally check the identity the file had before the scanner began reading embedded tags (`legacy_local_identity_key` / `local_track_is_blocklisted` in `playlist/local_library.py`, reproducing the old `stem.split(" - ")`-or-`Unknown` rule from the path alone), so a ban placed before that release still matches the same file after it — nothing on disk records the old key. The remaining gates keep checking the current identity only; they stay correct by containment, because a legacy-banned local file never enters `state.playlist` and a pre-upgrade norm-cache sidecar already carries the old identity. An untagged local file legitimately has `artist == ""`; that is a real identity `("", title)`, not a missing one, so `load_track_metadata` returns such a sidecar rather than `None` and the norm-cache rescue gate (`_is_blocklisted`) can no longer mistake it for the unidentifiable case and fail open. Admin endpoints: `POST /api/track/ban`, `POST /api/track/ban-now-playing`, `POST /api/track/unban`, and `GET /api/track/banlist`.
- **Moment Receipts (ritual-recipe moment trail).** `home/moment_receipts.py` (`MomentStore`, set on `state.moment_store` by `main.py`) records every authorized HA ritual-recipe moment from match to confirmed air: `elected`/`dropped(reason)` rows at the producer poll site; an opaque id stashed into the consuming banter's metadata (`ritual_moment_id`: a consumed directive's id rides the scriptwriter consume/restore handoff, never live state; an interrupt directive keeps its id in the pending slot until queue-commit, protected because competing matches drop as `*_slot_busy`; `gag_moment_id` for ritual-sourced evening-gag buckets via the `ritual_family` field threaded `HomeEvent → GagBucket`, which also upgrades the bucket label to the generic family label so a device name can never become a receipt label); a provisional `airing` flip in `on_stream_segment_audible()`, gated on a listener actually accepting the segment's first chunk rather than on selection or send-start (rescue/fallback fills guarded out too — backup audio never claims credit for the house); and a final status verbatim from `classify_stream_outcome` in the playback loop's finally (`_finalize_moment_receipts`, independent of the provenance ledger). A banter that airs canned demotes its gag row honestly (`canned_fallback`). Store is bounded (`cache_dir/moments.json`, cap 100, 7-day retention, `_CACHE_PROTECTED`, atomic write, corrupt-tolerant load); disk writes happen only at the producer save site — streamer paths are dirty-flag-only so the playback loop never does JSON I/O. Public surface: `ha_moments.recent` on `/public-status` (≤3 generic `public_family_label`s + coarse age, no entity ids/confidence/lines — the listener "Live from your home" strip in the Casa card; an `airing` row shows only while its segment matches `now_streaming`). Admin surface: `moments_admin` on `/status` (≤25 full rows — the "Home moments" panel in Motore, machine states mapped to plain words). Narrow installs do not project persisted receipt rows onto either surface. Every store call is best-effort — a receipt bug must never become an audio bug. See `docs/architecture.md` → "Moment Receipts".
- Operator triggers (`/api/trigger` banter/ad/news) get **air-next**: the producer builds the forced segment, then inserts it at the next priority position (`_front_insert_queue_and_shadow`, a no-await drain→insert→repush that keeps any already-queued urgent warning ahead and drops the furthest-future eligible tail if the bounded queue would overflow) so it airs at the next safe boundary instead of behind buffered lookahead. Only operator triggers (`operator_force_pending`) use this path; the 60s-silence rescue and other internal forces append normally. A second trigger while one is still pending is rejected with a way-out message (one at a time), and forced/urgent banter leaves listener requests for a normal break where the promised song can remain adjacent.
- `force_next` and `pinned_track` have revisioned ownership. Every semantic write — including assigning the same value again or clearing it — goes through the matching `StationState` setter and advances its revision; listener pins, admitted-song retries, and urgent-interrupt safety forces retain the revision they own, and cleanup uses guarded clears. A listener dedication may borrow a same-recording operator pin but cannot retract it. An urgent owner survives force claim and failed/stale rendering, and its dedicated producer slot runs ahead of buffered or newer operator work without erasing that work; Panic and Stop instead settle its directive, bridge, and Moment Receipt explicitly. Panic supersedes stale `/api/trigger` attribution as well as its force, then publishes recovery `MUSIC`; Stop clears all forces. A queued dedication handoff and its admitted request-exclusive music share the dedication queue id, so removing the announcement—or failing to read it before its first byte—also removes that music; a newer operator-owned same-recording segment survives with the request metadata stripped. Admission retains a token-keyed reservation until the promised segment reaches playback, so an unmarked equivalent buffered ahead cannot slip through after the request is archived. A coincident operator MUSIC action is fulfilled by that promised segment through normal admission behind its dedication, never by front-inserting it ahead of the announcement.
...
mammamiradio/
  ...
  release_campaign.py       packaged release-beat manifest -> bounded on-air cold-open campaign
...
- **Hero banner / social card**: `docs/banner.png` and `docs/social-card.png`, matching 1280×640 README hero and social preview. Warm Home Assistant-native room background composited with Playfair italic typography and the "La Casa Ti Ascolta" overlay. Source template: `docs/hero-composite.html` (contains regeneration instructions in comment header). The temporary background image (`banner-bg.png`) is generated externally and not committed to git.
...
- `scripts/conductor-*.sh` define Conductor workspace setup/run/archive behavior. The committed `.conductor/settings.toml` wires those hooks for new local and cloud workspaces after the change reaches the default branch. Machine-local `.conductor/settings.local.toml` values take precedence, so do not duplicate lifecycle hooks there. Keep `.context/` runtime state out of git.
...
- To tune host behavior without a stream gap: edit `mammamiradio/hosts/scriptwriter.py` (generation logic) and/or the prompt-data leaves — `mammamiradio/hosts/prompt_world.py` (expression banks, host fingerprints, exchange-shape/lore data, Chaos/Festival mode blocks), `mammamiradio/hosts/relationship.py` (roster-aware shape/lore selection and recency), `mammamiradio/hosts/transitions.py` (transition rewrite openers), `mammamiradio/hosts/fallbacks.py` (chaos stock lines, ad-break bumpers), `mammamiradio/hosts/station_name_guard.py` (foreign/competitor station-name scrubbing) — run `make check`, then `POST /api/hot-reload` (admin auth, empty body) — it reloads `language_policy`, `prompt_world`, `relationship`, `transitions`, `fallbacks`, `station_name_guard`, then `scriptwriter` leaves-first, so edits to any of them take effect — then `POST /api/trigger {"type": "banter"}` to generate a segment with the new code. The stream stays live throughout. If reload fails (syntax error), the endpoint returns 500 and the stream keeps running with the old code. `mammamiradio/hosts/memory_extractor.py` (post-air memory extraction) is deliberately excluded from hot-reload — it holds live in-flight task/apply-lock state that a reload would reset mid-extraction; edit it and restart to pick up changes.
...
- **Landing contract — human and feature PRs merge through `scripts/land-pr.sh`, never raw `gh pr merge` or `gh api` merge calls (single source of truth; the runbook links here)**: The sole automated exception is `.github/workflows/dependabot-automerge.yml` for eligible Dependabot patch and minor updates. `/ship` opens human and feature PRs and never arms auto-merge. The PR soaks (CodeRabbit, review time) until Florian's explicit merge signal. On the signal, `scripts/land-pr.sh <PR#>` (1) refuses a behind or conflicted branch before evidence verification and without mutating the PR, directing its feature workspace to integrate `origin/main` and run `scripts/emit-review-evidence.sh --reattest`; (2) requires committed v2 evidence covering the resulting up-to-date head
(the gate predicates live in `scripts/land-gates.sh`, shared with the
report-only shadow queue), with a current local gstack ledger as supplemental proof and as the mandatory fallback when the evidence check is explicitly skipped; (3) blocks unresolved current Major/Critical/P0/P1 bot threads and fails closed if thread data cannot be read; and (4) arms `gh pr merge --squash --auto --match-head-commit <head>` so GitHub merges only when required checks pass AND the head is still the one verified — a later push cancels the landing instead of shipping unseen code. The same hook denies raw `gh pr merge` (`--disable-auto` is allowed for disarming), REST `gh api` PUT calls to `/pulls/<n>/merge`, and GraphQL `gh api graphql` mutation payloads containing `mergePullRequest` or `enablePullRequestAutoMerge` (inline or loaded from an inspectable local file; stdin/unreadable payloads are denied because the hook cannot inspect them safely). Branch protection on `main` requires branches to be up to date before merging (strict status checks, set 2026-06-12) — this is what retires hand-rolled rebase/reset base-integration, the cause of the 2026-06-11 phantom-revert near-miss. Dependabot auto-merge is opportunistic, not a self-landing guarantee: a behind PR parks until an authenticated maintainer handles that specific PR. While `main` advertises an unpublished add-on version (the cut window, read from GHCR by `scripts/check-advertised-version.sh`), `dependabot-automerge.yml` disarms Dependabot auto-merge instead of arming it and labels the PR `cut-window-hold`; its sweep only disarms. Once the window closes, a fresh metadata-backed PR event or the landing workflow is needed to resume held PRs. PR-event arming pins the metadata head and refuses a changed main; manual disarm alone is not a persistent opt-out from PR events. Disarm/freeze the queue before a cut because asynchronous reconciliation cannot supply an atomic merge lock. GitHub Actions must not post Dependabot rebase or recreate commands; the batch-wide nudge was retired after its GitHub Actions actor was rejected by Dependabot, causing repeated comments and CI churn after subsequent merges. Settings drift tripwire: `scripts/check-merge-gate.sh` (in `make pre-release`), which reads GitHub's effective rules for `main` rather than reimplementing ruleset pattern matching. Honest scope: the hook is a local guard, not a security boundary (fail-open, bypassable via the GitHub UI), and exact-head reattestation **reduces** the stale-branch-claim class — it does not eliminate non-conflicting staleness.
- **Shadow land queue (report-only)**: `.github/workflows/land-queue.yml` runs
  ...
  and advance edge — plus why every other open PR is not that PR. It merges
  nothing, comments nowhere, and holds no write permission; it is **not** an
  exception to the Landing contract above. It reaches its verdict through the
  same predicates `land-pr.sh` arms on (`scripts/land-gates.sh`) and the same
  selection `cut-edge-release.sh` cuts on (`scripts/edge-select.sh`), so a
  shadow cannot drift from the thing it shadows; a test asserts the queue
  evaluates every gate `verify_head` arms on. Kill switches: delete
...
- **Advertised-version guard** (`scripts/check-advertised-version.sh`): asks GHCR whether the version `main` advertises to the HA Supervisor exists, for both arches, over the anonymous token endpoint (no scope needed). Home Assistant requires a prebuilt `image:` add-on's `version:` to name a real tag; when it does not, fresh installs fail and updates roll back.
  ...
  - Also read by `.github/workflows/dependabot-automerge.yml` on every Dependabot PR event, on the cut commit's push, and hourly: `fail` disarms Dependabot auto-merge (label `cut-window-hold`), `pass` permits verified patch/minor PR-event arming (the sweep never arms), `unknown` leaves state alone. Self-test: `bash tests/workflows/test_dependabot_automerge_gate.sh`.
...
- **Station name in copy** (`docs/design/system.md`): Write **Mamma Mi Radio** in prose. Use `mammamiradio` only in URLs, paths, package names, environment variables, entity IDs, and container names. `tests/web/test_ui_copy.py::test_station_brand_name_is_never_misspelled` scans tracked text files, including changelogs. Describe the typo without repeating it.
...
The *baseline* lives in `florianhorner/gh-workflows` and is shared by every repo. It matches the grammatical **shape** of a violation rather than a literal phrase — a review noun near a discovery verb, a review actor mid-action, a telemetry possessive, a numeric process tally, a review-artifact heading — so a paraphrase of a banned class does not walk through. A literal-phrase list cannot do this: the previous one held a pattern for `findings raised` and still passed "Findings from review". An em-dash rule ships alongside it and is now **blocking**, not advisory (gh-workflows v1.5, pinned in `.github/workflows/pr-body-lint.yml`). Two carve-outs keep it from contradicting the proof gate that runs on the same body in the same invocation: a line matching `n/a —` and any checkbox line (`- [ ]` / `- [x]`) are exempt, so a `## Proof` block never trips it. Only prose em-dashes fail.
...
## Perceptual features ship their A/B test first

Any feature whose only output is a perceptual change — audio colour, a visual
tweak, anything a human is supposed to *notice* — must ship the means to perceive
and compare it (an on-demand A/B, a side-by-side, a before/after sample) in the
SAME change, before or alongside the feature itself. If you cannot hear or see the
difference on demand, you cannot tune it, defend it, or catch its regressions, and
neither can an operator.

This rule was added 2026-06-18 after the FM "broadcast chain": a subtle always-on
egress colour that was (1) inaudible on the maker's own speakers, (2) impossible to
A/B because the live toggle only affected segments past a 5-15 min lookahead queue,
and (3) only ever audible as a stereo "binaural swirl" bug. The fix's own review
then found the chain was only loudness-neutral by accident. A feature you cannot
hear-test is unfalsifiable: it hides its own bugs until a listener finds them, and
it rots because no one can tell whether it still does anything. The corollary: when
