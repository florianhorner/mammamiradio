...
- **Keepsakes (durable "Keep this").** Clips expire after 24h (`CLIP_TTL_SECONDS`) and the provenance ledger prunes after `MAMMAMIRADIO_LEDGER_RETENTION_DAYS` (default 14), so a moment worth keeping is destroyed twice over by default. `POST /api/clip/keep` (admin auth, no body) is the on-air console's **Keep this** button: it exports the voice segment currently on air, or within `CLIP_LOOKBACK_SECONDS` the one that just ended via the `last_shareworthy_clip` snapshot (written for `AD`/`BANTER` only, so the other four eligible types have no lookback), to `cache_dir/keepsakes/` with a sidecar (`station_name`, `track_title`, `track_artist`, `segment_type`, `source`, `created_at`). Durability is structural, not a flag: `cleanup_old_clips` and the `CLIP_MAX_SAVED` cap operate on `clips/` alone, and every cache pruner globs `cache_dir/*.mp3` without recursing, so nothing that prunes the cache can see the directory; `cleanup_old_clips` additionally refuses outright when handed a directory named `keepsakes`, and `test_cache_eviction_never_reaches_into_keepsakes` guards the non-recursive assumption the other modules provide. **Two independent eligibility gates, both failing closed** (`is_keepsake_eligible` in `scheduling/clip.py`): the segment type must be in `KEEPSAKE_SEGMENT_TYPES` (banter, ad, news_flash, station_id, sweeper, time_check), **and** the segment must not carry `has_music_tail` — `commit_music_handoff` crossfades the outgoing song's real master under the opening seconds of the next voice segment, so type alone stopped being proof of provenance. A keepsake never expires and is served unauthenticated, so a tailed segment is refused rather than trimmed. The lookback snapshot carries the flag too and its own recorded type is re-checked, not trusted. Refusals name the actual situation (`music`, `music_tail`, `not_on_air`, `too_early`, `archive_full`, `no_room`) with a concrete next step, never a catch-all. The window is only what has aired of this segment: `/api/clip`'s 30s floor would reach back past the segment boundary, so anything under `KEEPSAKE_MIN_SECONDS` is refused as `too_early`. Bounded by `KEEPSAKE_MAX_SAVED` and a `KEEPSAKE_MIN_FREE_MB` free-space preflight, because this is the one cache directory nothing reclaims. The ring buffer is copied on the event loop and joined in a thread (`_snapshot_ring`), and the file write is also threaded, so a ~4 MB export cannot stall the playback sender against its 4s cushion; the write is `mkstemp`+`fsync`+`os.replace` so a kill mid-write leaves no permanent half-file in a directory with no evictor. Keepsakes are served through the existing `/clips/{id}.mp3` and `/clips/{id}` URLs (checked **after** `clips/`, so an id can never resolve to a keepsake while a live clip of that id exists) and skip the TTL check. Each keep records an `operator_action` ledger row when the ledger is enabled. Note that `POST /api/clip` (listener-facing) shares only complete bundled starter tracks since the copyright-safe sources work, so **Keep this is the only path by which host or ad audio leaves the station.**
