...
- **Moment Picker ownership:** the playback loop owns the retained byte ledger and segment marks; the producer alone stamps `segment.metadata["clip_audio_class"]`; the capture worker indexes complete frames and writes temporary sources; and the listener may submit only a frozen `choice_id`. Keep legacy `POST /api/clip` independent. Any capture/frame change must run `.venv/bin/pytest -m requires_ffmpeg tests/web/test_clip_capture_precision.py -v`; temporary `/captures/` audio must remain ingress-relative, `no-store`, and excluded from the service worker cache.
