...
- To tune host behavior without a stream gap: edit `mammamiradio/hosts/scriptwriter.py` (generation logic) and/or `mammamiradio/hosts/prompt_world.py` (expression banks, host fingerprints, Chaos/Festival mode blocks — the prompt-fiction data), run `make check`, then `POST /api/hot-reload` (admin auth, empty body) — it reloads `prompt_world` then `scriptwriter` leaves-first, so edits to either take effect — then `POST /api/trigger {"type": "banter"}` to generate a segment with the new code. The stream stays live throughout. If reload fails (syntax error), the endpoint returns 500 and the stream keeps running with the old code.
