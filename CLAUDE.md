...
- **Clip-capture ownership and rights boundary:** the playback loop owns the retained byte ledger and segment marks, while the producer alone stamps `segment.metadata["clip_audio_class"]`. Capture publishes only a continuous voice run classified as `speech` or `station_bed`; commercial, unknown, mixed, and otherwise unproven material is rejected. The worker freezes server-owned choices and produces decoder-safe output from bounded source context. Temporary `/captures/` audio is ingress-relative, capability-scoped, `no-store`, excluded from the service worker cache, and bounded by per-IP quota and expiry. Capture state resets at each application lifespan, and capture commits share the legacy clip route's serialized 24-hour/50-file retention boundary. Any capture/frame change must run `.venv/bin/pytest -m requires_ffmpeg tests/web/test_clip_capture_precision.py -v`.
