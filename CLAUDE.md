...
- Source switching via `/api/playlist/load` cuts over immediately only when fresh
  ...
  promise it no longer owns. If the dedication is already on air and its exact
  admitted song is ready in the queue, preserve that song instead of unrelated
  old-source runway and retain its first-byte ownership until it starts; the
