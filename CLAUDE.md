...
- `MAMMAMIRADIO_CACHE_DIR`, `MAMMAMIRADIO_TMP_DIR`, `MAMMAMIRADIO_MUSIC_DIR`: override cache, temporary-work, and operator-supplied music directories. Supplied standalone containers use `/data`; the Home Assistant add-on keeps cache and temporary work there but reads new music from `/media/mammamiradio`, with `/data/music` retained as a legacy discovery root.
...
- **Version sync check** (inline in `quality.yml`): runs on PRs that touch `pyproject.toml` or `ha-addon/mammamiradio/config.yaml`. Runs the full `scripts/pre-release-check.sh` (version consistency + CHANGELOG head + all invariants). No-ops on unrelated PRs. Local: `make pre-release`.
  - Step 9 (physical HA Green 20-run receipts) is scoped to a real release cut: it runs when `ha-addon/mammamiradio/config.yaml`'s `version:` differs from the one on `origin/main`, or when `proof/media/ha-green-release-evidence/run-*.json` receipts already exist. A PR that only touches an add-on option or a manifest key cannot produce Green hardware receipts, so asking for them there would deadlock it — the same reasoning as the advertised-version guard below. The unconditional publishing gates stay in `addon-release.yml` (promote) and `docker.yml` (tag push). Guarded by `tests/repo/test_ha_green_release_gates.py`.
