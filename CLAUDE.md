...
- **Listener-request station identity is candidate-backed and request-independent.** Before a verified search result becomes a `Track`, conventional `feat.`/`ft.`/`featuring` credit placement is collapsed to one base artist/title identity: `Lady Gaga feat. Bradley Cooper – Shallow` and `Lady Gaga – Shallow (feat. Bradley Cooper)` both enter rotation as `Lady Gaga – Shallow`. The full candidate credit and verified collaborators remain admission-time blocklist aliases, so requesting a collaborator cannot bypass an existing ban; listener spelling never supplies station or policy identity.
...
- `force_next` has revisioned ownership. Every semantic write — including assigning the same segment type again or clearing it — goes through `StationState.set_force_next()` and advances `force_next_revision`; cleanup that owns a temporary force uses `clear_force_next(expected_revision=..., expected_type=...)`. This prevents a stale listener-request render from clearing a newer same-valued `MUSIC` force published by Panic or another control. `operator_force_pending` remains the separate `/api/trigger` attribution slot.
