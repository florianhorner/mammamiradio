...
- **Release invariants** (`scripts/check-release-invariants.sh`): runs on every PR. Catches (1) FFmpeg `music_eq_chain` equalizer count ≠ 2 (Pi aarch64 SIGABRT risk), (2) missing packaged recovery MP3 or producer-side `generate_silence`, (3) missing `_pick_canned_clip=None` test mock (missing packaged recovery untested), (4) missing `session_stopped` test (post-restart silence untested). Local: `bash scripts/check-release-invariants.sh`.
