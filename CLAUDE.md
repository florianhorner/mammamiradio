...
- Source switching via `/api/playlist/load` cuts over immediately only when fresh
  ...
  whether that song is admitted or still rendering: keep admitted music at the
  queue head, or restore an active handoff with fresh force/pin revisions after
  the old render is fenced stale. In the active case, hold fresh or preserved
  fallback audio in the capacity-exempt slot so the retry can queue first; the
  slot/playback recovery ladder covers a late retry. The dedication finishes and
  the response remains `skipped: false`.
