...
- **Metadata-only ingest commits.** Every slow playlist-mutating route captures
  `continuity_epoch` before its awaits and re-checks it at the commit boundary.
  When the station is stopped, or the epoch moved while the request was in flight
  (a Stop, or a Stop plus a fast Resume), the route still applies the
  playlist/track/heading change but skips runway reservation entirely and returns
  `metadata_only: true` — metadata moves, no audio is admitted, nothing is skipped.
  Applies to `/api/playlist/load`, `/api/playlist/enrich`, `/api/heading`,
  `/api/direction`, and the external/listener download commit. The normal producer
  replenishes audio after a valid Resume.
