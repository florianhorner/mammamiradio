...
- `audio.broadcast_chain` (default `true`) is the **egress pipeline's** always-on final stage: every aired segment passes through one funnel (`_enqueue_with_egress` in `scheduling/producer.py`), and `apply_broadcast_chain()` in `audio/normalizer.py` runs one extra FFmpeg pass that colours it like an over-the-air FM signal (subtle multipath, gentle pre-emphasis HF shelf, ~15 kHz band-limit, soft leveller). Voice and music exit through the same stage, so there is no FM-music-next-to-studio-clean-voice seam. Configured at startup via `configure_broadcast_chain()`. A separate pass with **no `loudnorm` and no `equalizer` in-graph** keeps the psymodel SIGABRT surface (3 equalizers + loudnorm on ffmpeg 8.x / Pi aarch64) closed; it holds the same `_NORM_SEM` slot as `normalize()` (Pi 2-FFmpeg ceiling), is loudness-neutral (so it never moves a segment off the reconciled target), and is best-effort (a failure airs the un-coloured audio — never dead air). Emergency / bridge / rescue / canned fills (`_is_rescue_fill`, truthy rescue marker) **skip** the pass so a dead-air rescue is never delayed (#2 INSTANT AUDIO). The chaos and reactive-interference content stages slot in **before** the broadcast stage.
