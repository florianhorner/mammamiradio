...
- **Moment Picker ownership and rights boundary:** the playback loop owns the retained byte ledger and segment marks. The producer alone stamps `segment.metadata["clip_audio_class"]`. Capture publishes only a continuous voice run that the server classifies as `speech` or `station_bed`; commercial, unknown, mixed, and otherwise unproven material is rejected. If no safe voice run is available, the listener falls back to legacy `POST /api/clip` for one complete validated bundled starter track. The capture worker freezes the choices and produces decoder-safe output from bounded source context. It may use preroll or re-encoding when needed, but neither mechanism is part of the API contract. The listener submits only a frozen `choice_id`, and final metadata comes from that choice. Temporary `/captures/` audio remains ingress-relative, capability-scoped, `no-store`, and excluded from the service worker cache. A per-IP concurrent quota limits captures, and `DELETE /api/clip/capture/{capture_id}` releases them. Capture records, quota accounting, retained timeline state, and generation ownership reset at every application lifespan, including repeated lifespans in one process. Moment commits and legacy clip creation share one serialized 24-hour/50-file final-retention boundary. Any capture/frame change must run `.venv/bin/pytest -m requires_ffmpeg tests/web/test_clip_capture_precision.py -v`.
...
- **Keepsakes (durable "Keep this").** Clips expire after 24h (`CLIP_TTL_SECONDS`) and the provenance ledger prunes after `MAMMAMIRADIO_LEDGER_RETENTION_DAYS` (default 14), so a saved moment can disappear from both records by default. `POST /api/clip/keep` (admin auth, no body) is the on-air console's **Keep this** button: it exports the voice segment currently on air, or within `CLIP_LOOKBACK_SECONDS` the one that just ended, to `cache_dir/keepsakes/` with a sidecar (`station_name`, `track_title`, `track_artist`, `segment_type`, `source`, `created_at`). Durability comes from the directory layout: `cleanup_old_clips` and the `CLIP_MAX_SAVED` cap operate on `clips/` alone, and every cache pruner globs `cache_dir/*.mp3` without recursing, so nothing that prunes the cache can see the directory; `cleanup_old_clips` additionally refuses outright when handed a directory named `keepsakes`, and `test_cache_eviction_never_reaches_into_keepsakes` guards that non-recursive boundary. The playback loop writes `app.state.clip_segment` (`{type, chunks, title, has_music_tail}`) at every segment boundary and increments `chunks` for each chunk that segment puts into the share ring. The route cuts those exact chunks (`extract_segment_audio` in `scheduling/clip.py`) instead of estimating length from elapsed time. The ring holds voice segments back to back; music never enters it. A window one chunk too long would therefore attach the previous segment's audio to this one's title and rights check. The same count sizes the `last_shareworthy_clip` lookback snapshot, which is written for all six keepable types. `clip_segment` is cleared on stop. The segment type must be in `KEEPSAKE_SEGMENT_TYPES` (banter, ad, news_flash, station_id, sweeper, time_check), and the segment must not carry `has_music_tail`; `commit_music_handoff` can place the outgoing song under the opening seconds of the next voice segment, so type alone does not prove provenance. A keepsake never expires and is served without auth, so a tailed segment is refused rather than trimmed. The snapshot carries the flag and its recorded type is checked again. The lookback is consulted only when the airing segment is not keepable; `too_early` and `music_tail` describe the current segment, and using the previous one would save audio the operator was not hearing. Refusals use the concrete reasons (`music`, `music_tail`, `not_on_air`, `too_early`, `not_keepable`, `archive_full`, `no_room`, `write_failed`) and include a next step. Audio shorter than `KEEPSAKE_MIN_SECONDS` is refused as `too_early`. `KEEPSAKE_MAX_SAVED` and the `KEEPSAKE_MIN_FREE_MB` preflight are checked with publication under `_keepsake_write_lock`, so two presses cannot both pass the ceiling. The ring buffer is copied on the event loop and joined in a thread (`_snapshot_segment`); the file write also runs in a thread, so a roughly 4 MB export does not stall the playback sender against its 4s cushion. The write uses `mkstemp` + `fsync` + `os.replace`, so a kill mid-write cannot publish a truncated keepsake. A leftover `.keepsake-*.tmp` file is ignored by serving, listing, and capacity checks, then removed at startup by `prune_stale_keepsake_tmp_files` after 6h. **Revocation.** `GET /api/clip/keep` lists the shelf and `DELETE /api/clip/keep/{id}` removes the audio and sidecar (admin auth, `operator_action` row `unkeep_this`). The admin Archivio tab shows **Kept moments** and an Undo action. Deleting the file revokes the keep because both public routes read from disk per request. Keepsakes use the existing `/clips/{id}.mp3` and `/clips/{id}` URLs after `clips/`, so a live clip with the same id always wins. They skip the TTL check. The console builds the link with the ingress-aware `_base` prefix and reports "copied" only after the clipboard write resolves. Each keep records an `operator_action` ledger row when the ledger is enabled. Legacy `POST /api/clip` shares only complete bundled starter tracks. The Moment Picker publishes only server-proven speech or station-bed voice runs as ordinary expiring clips. **Keep this remains the only durable, no-TTL path by which host or ad audio leaves the station.**
