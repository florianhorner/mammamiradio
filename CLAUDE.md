...
- **Metadata-only ingest commits.** Slow playlist/source, heading, enrich, and
  ...
  normal producer replenishes audio after a valid Resume. Those routes also return
  `resume_required: true` when the reason was a paused station, which is the
  narrower half of `metadata_only` (a mid-flight epoch move sets `metadata_only`
  without it). The admin reads it to say the change is saved but nothing new airs
  until Start, so an operator is never told a deferred change landed on air.
