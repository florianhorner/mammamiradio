...
- **Explainer guards**: `.github/workflows/explainer.yml` runs the
  `docs/explainer/` test suite (`npm test` + `npm run build`) on any PR
  touching `docs/explainer/**` — the framing guards that keep the page
  station-first, plus the scenario/build validation. String-level only;
  playback behavior stays with the pure-function phase tests and the manual
  steps in `docs/explainer/README.md`.
