...
6. **Physical proof binds complete cut content.** Finalize version, changelogs, and V1/V2
   before HA runs. Its digest survives squash and rejects all other drift; `source_commit` is provenance.
...
- **Preship V2 comes before HA runs.** It excludes validated HA receipts; HA includes V2
  and excludes only HA receipts, keeping both proofs squash-safe.
