...
2. **`_pick_canned_clip=None` test mock**: at least one test file must mock this to `None`. Tests that return a real file hide the empty-container recovery scenario that happens in production (Pi container ships only README stubs in `mammamiradio/assets/demo/banter/`).
...
6. **Producer error-recovery rescue ladder**: `mammamiradio/scheduling/producer.py` must not import or call `generate_silence`. The producer's exception-recovery path rescues with real audio (canned clip → norm-cache track → last-known-good music → recovery sweeper → emergency tone), never generated silence — this invariant fails CI if that regresses.
