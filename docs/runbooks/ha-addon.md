...
To check whether the window is open right now, run `scripts/check-advertised-version.sh`. `advertised-version.yml` runs it daily. `dependabot-automerge.yml` reads current `main` on Dependabot PR events, on a cut commit's push, hourly, and on demand. A missing published image disarms eligible Dependabot PRs and marks successfully recorded holds with `cut-window-hold`; an unreachable registry leaves auto-merge unchanged. PR-event arming requires verified patch/minor metadata, the same PR head, and an unchanged `main` since the probe. The sweep only disarms: after publication, trigger a fresh metadata-backed event with `@dependabot rebase` from a maintainer account or use the landing workflow; major updates always need manual landing. Label events retain the normal opportunistic arming behavior, so a manual disarm alone is not a persistent opt-out.

The workflow serializes its own runs, but it cannot atomically block a merge while waiting for a runner or GitHub. Keep strict up-to-date branch checks and disarm/freeze the dependency queue before landing the cut. A green reconciliation run is not proof of a release freeze.
