...
`mammamiradio/scheduling/producer.py` currently mixes four different responsibilities inside `run_producer()`:
...
That shape is now the main fragility point in the app. The immediate operational pressure is Pi queue starvation and dead air, but the underlying design pressure is that producer behavior is coupled to `StationState`, `mammamiradio/web/streamer.py`, and several UI contracts.
...
The split lives entirely inside the `scheduling` nave so the cathedral's nave boundary stays intact. Proposed internal package:

- `mammamiradio/scheduling/production/types.py`
- `mammamiradio/scheduling/production/commit.py`
- `mammamiradio/scheduling/production/policy.py`
- `mammamiradio/scheduling/production/music.py`
- `mammamiradio/scheduling/production/banter.py`
- `mammamiradio/scheduling/production/ads.py`
- `mammamiradio/scheduling/production/fillers.py`
...
- create `mammamiradio/scheduling/production/music.py`
...
- create `mammamiradio/scheduling/production/policy.py`
- move:
  - idle and resume behavior
...
- create `mammamiradio/scheduling/production/banter.py`
- move:
...
- create `mammamiradio/scheduling/production/ads.py`
- create `mammamiradio/scheduling/production/fillers.py`
- move:
  - ad break assembly
...
- `mammamiradio/scheduling/producer.py` is a thin orchestrator
...
Tests mirror source per the cathedral layout (`tests/<nave>/test_*.py`).

Minimum suite per phase:

- `tests/scheduling/test_producer_unit.py`
- `tests/scheduling/test_producer_extended.py`
...
- `tests/scheduling/test_production_music.py`
- `tests/scheduling/test_production_policy.py`
- `tests/scheduling/test_production_banter.py`
- `tests/scheduling/test_production_ads.py`
- `tests/scheduling/test_production_commit.py`

Per-module coverage floors in `.coverage-floors.json` must land with each new module per the project's coverage ratchet rule. The current `mammamiradio.scheduling.producer` floor is 83 — extractions must raise the aggregate, not lower it.

## Success Criteria

The refactor is succeeding if:

- `mammamiradio/scheduling/producer.py` becomes smaller because responsibility moves out, not because behavior is hidden in harder-to-follow helpers
