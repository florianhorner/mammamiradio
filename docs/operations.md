...
### Il Bollettino di Casa

`home_bulletin` is an earned 30–45 second voice programme, presented on air as
**Il Bollettino di Casa**. It becomes eligible once a listener has accepted the
first audio chunk from each of three music segments in the current
listener-session epoch (a new epoch begins after at least ten listener-free
minutes), and it can air only once in that epoch. Rendering also requires one
currently eligible prompt-safe Home Context Director fact and a ready live
script provider. If either prerequisite disappears, generation fails, the
listener epoch changes, or no listener accepts the bulletin's first chunk, the
station releases the reservation and returns to ordinary scheduling. It does
not emit a canned or generic Casa bulletin and does not immediately retry in a
loop.

**Compatibility release gate:** `home_bulletin` is a new raw segment type on
`/status`, `/public-status`, and `sensor.mammamiradio_segment_type`; the same raw
enum also reaches the frozen v1 integration feed. Do not merge, enable, or
release this programme until the `CONTRACT.md` contract window has authorized
the v1 change and its provider-first fixture sequence is complete. The feature
implementation does not itself modify the frozen integration surface.
