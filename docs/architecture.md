...
| `/api/hot-reload` | POST | Admin | Reload `prompt_world.py` then `scriptwriter.py` (leaves-first) in-place via `importlib.reload()` — stream continues uninterrupted, next banter uses new code. Requires `--workers 1`. |
