...
5. Restores an eligible persisted base selection without scanning local files; otherwise the hash-pinned attributed starter catalog is the offline base. A retired `jamendo://` source is rewritten, and add-on-external selections cannot restore extractor authority. Local discovery starts after audio and overlays future rotation. `playlist_source.kind` names what is in the crate: once the scanner overlays operator files, kind becomes `local` (local is the base) even though boot loaded the starter bag. Mixed starter+local pools use weighted selection with starter no-repeat reservations; a genuinely starter-only pool keeps bag order. The continuity-bridge recovery rung asks the selector with an explicit `restrict_to_source="starter"` so it still returns starter media when locals would otherwise win.
