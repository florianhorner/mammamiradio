...
that, the source-to-fanout schedule stays no more than one packet (4.125 seconds)
ahead. This absorbs the 1.781-second scheduler stall measured on HA Green before
it reaches a direct listener (for example, a Sonos player consuming `/stream`).
...
give a slow listener a tighter time budget before drop. The 4,000/4,125 ms bound
