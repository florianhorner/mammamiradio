...
- **the missing-unit policy.** Callers pass `require_unit=True` wherever guessing wrong would state a wrong number as fact: classified temperature sensors (in **both** `_format_state` and `DirectorObservation.from_home_assistant_state`, so the boundary is self-enforcing rather than relying on one module filtering for the other), the weather arcs, and `authorization._temperature_c`. **Fallback path:** an unresolvable unit withholds the number rather than assuming Celsius — a weather/climate line keeps its condition or mode, and a unitless temperature sensor is dropped entirely. `weather.*` state lines and `climate.*` keep the legacy Celsius assumption because HA publishes no unit for them; `climate.*` is the known gap (HA pre-converts climate values into the household's configured unit and publishes no unit attribute), pinned by `test_format_state_climate_without_a_unit_attribute_reads_as_celsius` and stated plainly in the release notes rather than implied fixed.
