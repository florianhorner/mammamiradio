...
5. Restores an eligible persisted base selection without scanning local files; otherwise the hash-pinned attributed starter catalog is the offline base. A retired `jamendo://` source is rewritten, and add-on-external selections cannot restore extractor authority. Local discovery starts after audio and overlays future rotation. Tag reads are memoized per file on `(size, mtime)` and the whole pass carries a metadata-probe budget, so the 60s scan loop does not re-run `ffprobe` over an unchanged library; an exhausted budget or a failed probe reports the scan incomplete and reconcile keeps what it already knows rather than overwriting tags or a measured duration with a guess. `playlist_source.kind` names what is in the crate for the bag kinds that carry no provider of their own (`starter`, `demo`, empty): once the scanner overlays operator files, kind becomes `local` even though boot loaded the starter bag. A kind that also selects a backing provider — `charts`, `url`, `jamendo`, `classic` — is left alone, because the mid-session chart refresh, the source loader, and the persisted-source restore all branch on it; `playlist_source.local_overlay` carries the mixed-crate fact there instead. Selection reads composition from `Track.source`, never from `kind`. Mixed starter+local pools use weighted selection with starter no-repeat reservations; a genuinely starter-only pool keeps bag order. The continuity-bridge recovery rung asks the selector with an explicit `restrict_to_source="starter"` so it still returns starter media when locals would otherwise win.
