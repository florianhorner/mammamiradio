...
- ad breaks trigger when `songs_since_ad >= songs_between_ads`, **and only when the station can actually make one**. `ad_programme_block()` in `producer.py` names what is missing — no ad brand that passes its voice-cast check (the only brands the producer will air), or no working AI key (none configured, none with a model route, or every configured key refused by its provider), checked in that order because with nothing to advertise a key would not help — and `ad_programme_available()` is true only when neither is. The producer refreshes it at the top of every loop tick, including while it idles with a full queue or no listeners, and `next_segment_type_for()` refreshes it again before each pacing decision, so an AI key added or cleared in Motore mid-session is reflected within a second. When the answer is no, the slot becomes music and `songs_since_ad` is **held at the threshold rather than reset**: the break stays owed instead of forgiven, fires the moment a real advertisement is possible, and the counter stays bounded instead of climbing without limit on a station that has no key. `/status` reports the reason as `pacing.ad_block`, and the admin shows **Ads paused** with the fix in place of the counter, which would otherwise read "— next" forever. `prewarm_first_segment()` settles the same capability as its first step on every boot path, including a stopped session and an empty playlist, because the schedule preview is served from startup while a First Listen install holds the producer's first decision for its opening. The preview feeds the integration's up-next, so that list never promises a break the station cannot make. The manual **Ad break** control bypasses pacing, so `/api/trigger` refuses an ad it cannot make with the specific fix (add or check the AI key in Motore, or check the ad brands and their campaign voices so at least one can air), ahead of the one-at-a-time check since waiting cannot fix either. A key can still be cleared between the tap and the render, so the producer re-checks before consuming any forced ad (`_drop_unmakeable_forced_ad()`): it drops the pick, releases the one-at-a-time guard, and restores an operator pick the force had displaced. Before this, a fresh install with no key aired `"<brand>. <tagline>"` as a finished advertisement on every break. **Scope:** a station that *has* a key still falls back to that same copy when generation fails (a provider error, a timeout, a dropped spokesperson role, a pharma spot left with only fine print, or the Normal Mode language check); that path is unchanged here. A key the provider has definitively refused is different: that verdict is sticky until the key is replaced, so it counts as no key and ads pause instead. A quota, rate-limit or network failure never sets it, and those still reach the ad writer
...
Packaged advertisements are a declared category in the fail-closed spoken manifest (`mammamiradio/assets/demo/ads/`, `core/spoken_assets.py`). Like banter they are mode-bound — `normal` is recorded in English, `super_italian` in Italian — and unlike banter they carry no adjacency: an ad never depends on the song before it and has no rare-special tier. Each entry must sit directly in `ads/`, with no subfolders, and declare a `duration_seconds` of **25–40 seconds**, inclusive. That bound is the mechanism, not a guideline: the copy that caused this rule rendered successfully and was two sentences long, and nothing in the packaging contract could object because the contract had no opinion about how long an advertisement is. The manifest checks the **declared** length; comparing it against the audio itself lands with the recordings, as the release validator already does for two other categories. Only ads read the field: other categories ignore a missing or malformed duration as they always did, so a typo cannot drop a recovery clip from the rescue ladder. Recording the pack is separate work, and so is the path that airs it — nothing airs from `ads/` yet.
