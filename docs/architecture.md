...
- ad breaks trigger when `songs_since_ad >= songs_between_ads`, **and only when the station can actually make one**. `ad_programme_block()` in `producer.py` names what is missing — no ad brands, or no AI key, checked in that order because with nothing to advertise a key would not help — and `ad_programme_available()` is true only when neither is. `next_segment_type_for()` refreshes it before every pacing decision, so an AI key added in Motore mid-session fires the owed break. When the answer is no, the slot becomes music and `songs_since_ad` is **held at the threshold rather than reset**: the break stays owed instead of forgiven, fires the moment a real advertisement is possible, and the admin's songs-since-last-ad counter stays bounded instead of climbing without limit on a station that has no key. `prewarm_first_segment()` settles the same capability as its first step on every boot path, including a stopped session and an empty playlist, because the schedule preview is served from startup while a First Listen install holds the producer's first decision for its opening. The preview feeds the integration's up-next, so that list never promises a break the station cannot make. The manual **Ad break** control bypasses pacing, so `/api/trigger` refuses an ad it cannot make with the specific fix (add an AI key in Motore, or add ad brands), ahead of the one-at-a-time check since waiting cannot fix either. Before this, a fresh install with no key aired `"<brand>. <tagline>"` as a finished advertisement on every break. **Scope:** a station that *has* a key still falls back to that same copy when generation fails (a provider error, a timeout, a dropped spokesperson role, a pharma spot left with only fine print, or the Normal Mode language check); that path is unchanged here
...
Packaged advertisements are a declared category in the fail-closed spoken manifest (`mammamiradio/assets/demo/ads/`, `core/spoken_assets.py`). Like banter they are mode-bound — `normal` is recorded in English, `super_italian` in Italian — and unlike banter they carry no adjacency: an ad never depends on the song before it and has no rare-special tier. Each entry must declare a `duration_seconds` of **25–40 seconds**, inclusive. That bound is the mechanism, not a guideline: the copy that caused this rule rendered successfully and was two sentences long, and nothing in the packaging contract could object because the contract had no opinion about how long an advertisement is. The manifest checks the **declared** length; comparing it against the audio itself lands with the recordings, as the release validator already does for two other categories. Recording the pack is separate work, and so is the path that airs it — nothing airs from `ads/` yet.
