...
| `/api/search` | GET | Admin | Always searches the local/base playlist; standalone `external-media` may add paginated external results, while add-ons return no extractor results |
| `/api/heading` | POST | Admin | Steer the next music stretch from matching base tracks; an era import requires standalone `external-media`, with add-ons failing external-only work closed |
| `/api/direction` | POST | Admin | Free-text direction over matching base tracks; only standalone `external-media` may resolve and download new targets in background |
...
- missing/disabled `external-media` leaves the local-or-starter base untouched; external-only standalone operations report the capability boundary, and add-ons return the locked actionable `403` without importing or executing `yt-dlp`
- Jamendo discovery, license, URL, network, normalization, timeout, race, cancellation, or restart failures destroy the single-use artifact/lease and immediately continue with local/starter music
