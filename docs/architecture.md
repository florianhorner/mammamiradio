...
- the weather news flash grounds itself in the real Home Assistant forecast when available (`state.ha_weather_arc`), then spins it into absurd Italian color; with no forecast (HA disconnected or unsupported) it falls back to the fully fictional meteo prompt, so the segment never goes silent
