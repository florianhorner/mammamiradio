...
- A hard kill between a spool write's `mkstemp` and its final `os.replace` can leave an orphaned `.manifest-*.tmp` (in `cache/restart_handoff/`) or `.handoff-*.tmp` (in `cache/restart_handoff/segments/`) scratch file behind. `prune_stale_handoff_tmp_files` sweeps both directories at the start of every boot (before `_admit_restart_handoff` runs), deleting only scratch files older than 6h, with path-containment and symlink checks so a corrupted/symlinked cache dir degrades to a no-op rather than raising, and a per-directory cap (500 candidates, oldest first) so a pathological backlog can't stall startup.
