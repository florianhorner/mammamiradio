...
5. Restores an eligible persisted base selection without scanning local files; otherwise the hash-pinned attributed starter catalog is the offline base. A retired `jamendo://` source is rewritten, and add-on-external selections cannot restore extractor authority. Local discovery starts after audio and overlays future rotation. Tag reads are memoized per file on `(size, mtime_ns)` under a lock (two threads reach the scanner: the 60s loop and producer local-rotation recovery), so the loop does not re-run `ffprobe` over an unchanged library; the memo is process-local, so the first pass after a restart re-reads everything, which is the pass the whole-scan probe budget exists to bound. A file the budget could not reach is *deferred* — absent from `tracks`, listed in `deferred_paths` — so reconcile neither admits it under a filename guess nor treats it as gone, and later passes converge. A probe that fails for any other reason is excluded from `probe_ok_paths`, so reconcile keeps the track it already has rather than overwriting its tags or a measured duration with a guess, and judges a ban on the identity it already knows for that file rather than on the guess. `playlist_source.kind` names what is in the crate for the bag kinds that carry no provider of their own (`starter`, `demo`, `local`, empty): once the scanner overlays operator files, kind becomes `local` even though boot loaded the starter bag, and a complete scan that finds the locals gone returns kind to `starter` when starter media remains. A kind that also selects a backing provider — `charts`, `url`, `jamendo`, `classic` — is left alone, because the mid-session chart refresh, the source loader, and the persisted-source restore all branch on it. Nothing else records the mixed-crate fact today; selection does not need it, because it reads composition from `Track.source`, never from `kind`. Mixed starter+local pools use weighted selection with starter no-repeat reservations; a genuinely starter-only pool keeps bag order. The continuity-bridge recovery rung asks the selector with an explicit `restrict_to_source="starter"` so it still returns starter media when locals would otherwise win.
