...
`rejections` counts first language failures that trigger a repair; `failures` counts terminal or final-text language failures. Both reset on restart. Provider failures and JSON parsing errors do not increment them. Normal Mode transitions accept an English marker followed by an Italian handoff; banter, news and ads keep their existing English floors. This is a word-marker heuristic: English words in titles or artist names also count. Normal Mode has four stock exchanges selected at random, so consecutive repeats remain possible.

Home label requests allow 1,200–4,000 output tokens, scaled to the candidate count. A truncated response gets one retry with the first half of the batch, for at most two application-level requests (SDK retries are unchanged). Each call keeps the 45-second timeout and rechecks the original privacy permission. A second truncation preserves the catalog; successful labels leave the pending set so later polls advance. Persistent truncation does not guarantee progress. Logs contain counts and fixed failure categories, never household output. Memory extraction starts at 900 output tokens and ads at 1,100; their existing truncation escalation still applies.
