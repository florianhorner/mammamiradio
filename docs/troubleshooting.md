...
To inspect script-side OpenAI behavior (banter/ads/news/transitions), grep logs for `openai_script_call` — every OpenAI script call emits a structured record with `model`, `caller`, `latency_ms`, `prompt_tokens`, `completion_tokens`, `json_ok`, and `fallback_reason` (one of `anthropic_absent`, `anthropic_auth_blocked`, `anthropic_auth_failed`, `anthropic_max_tokens_truncated`, `anthropic_nonretryable`, `anthropic_usage_limit`, `anthropic_usage_limit_blocked`, `anthropic_exception`). `anthropic_max_tokens_truncated` means the Anthropic response was cut off at the token budget (partial or empty JSON) — since the escalation retry landed, that terminal reason is normally preceded by ONE in-house retry at a ~1.75× budget (ledger reason `anthropic_max_tokens_truncated_retrying`, log line `Anthropic truncated at max_tokens=…; retrying with escalated budget …`), and only after the retry also truncates does the call fall back to OpenAI, whose visible-output floor then inherits the escalated (not original) budget. The OpenAI side has its own single retry: an empty completion or `finish_reason="length"` (gpt-5.x reasoning tokens starving the visible JSON) retries once with a bigger cap (attempt reason `openai_empty_or_length`) before the stock-copy fallback. When hosts sound generic, grep for `anthropic_max_tokens_truncated`, `openai_empty_or_length`, and the early-warning `budget pressure` WARNING (fires when a successful generation used ≥80% of its budget — raise the budget before the next truncation, don't wait for it). Useful for comparing models via `OPENAI_SCRIPT_MODEL` or debugging fallback latency.
