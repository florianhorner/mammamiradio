...
OpenAI script calls now have a separate bounded breaker. Authentication, quota, routing, network, timeout, and server failures pause further OpenAI script attempts until the cooldown ends; a successful response clears the pause. Stock host copy keeps the station moving when both AI paths are unavailable. Saving the OpenAI key clears its old breaker, and an earlier key's late failure cannot disable the replacement. OpenAI speech has its own health state.
