...
:root {
  ...
  /* Motion — track-title content swap only; NOT --transition-slow (320ms reads clipped, needs read-time) */
  --transition-track: 600ms cubic-bezier(0.16, 1, 0.3, 1);
}
```
...
## Language scope — Super Italian Mode

The station has two language modes, set by the operator (admin Engine Room → "Super Italian Mode" toggle, or `MAMMAMIRADIO_SUPER_ITALIAN` env var). It is a **personality dial**, not a translation toggle: it changes listener copy and AI host behavior. The admin stays English-first in both modes.

| | Super Italian (ON) | Default Reduced Italian (OFF) |
|---|---|---|
| Listener UI — headlines, section names, brand idioms (`Stasera in onda`, `Palinsesto`, `Dediche & Saluti`, `Mi`, tricolor) | Italian | Italian (always) |
| Listener UI — buttons, form placeholders, body labels (`Listen Now`, `Tracks in Rotation`, `Your name`) | Italian | English |
| Listener UI — JS dynamic labels (`now`, `min ago`, `Music`, `Banter`) | Italian | English |
| Admin UI | English-first operator copy | English-first operator copy |
| AI host language | 100% Italian, addresses listeners as `amici miei` — only song/artist/brand names and the guest host's Bavarian phraselets exempt | Roughly 75% English / 25% Italian: English carries the information; Italian carries the heart (greetings, exclamations, punchlines) including occasional complete Italian sentences. The 75/25 split is what the prompt asks for; enforcement is one-sided — copy that runs Italian-heavy is rewritten once, copy that runs English-heavy still airs in full rather than collapsing to stock |
| Banter / ad / song-cue *content* generated by hosts | Driven by `[brand].language` (separate concern, not this toggle) | Same |
...
| 2026-09-06 | The admin live deck's sticky backdrop is state-driven, and deviates from the sticky-nav treatment in two named ways | The deck sits where the Sun Glow atmosphere is warmest, so the flat `var(--bg)` fill it used to paint permanently stamped a hard-edged rectangle over the glow and grain, measured as a 17/255 step at its bottom edge. It is now transparent at rest and opaque only while pinned. Two deviations from the listener sticky-nav spec, both measured: (1) no `backdrop-filter: blur(12px)`, because behind the admin deck it is espresso on espresso, and blurred vs flat differed by 6/255, which does not pay for a compositing layer on a 271px element that repaints every scroll frame; the blur keeps earning its place on the listener page, where artwork and the brighter `--card*` surfaces pass underneath. (2) no 1px `--line` bottom border, because the tab bar already ends in its own `--line-strong` rule 16px above the deck's edge, so the hairline rendered as a railroad track; the pinned cue is `box-shadow: 0 6px 16px -12px rgba(0,0,0,.85)` instead, which reads as elevation without a competing line. The fill is excluded from the `transition`: the tab bar has no background of its own, so a cross-faded fill would let the page scroll through the tab row for the length of the fade. That narrows the bleed to each pin crossing instead of removing it. |
