...
The station has two language modes, set by the operator (admin Engine Room → "Super Italian Mode" toggle, or `MAMMAMIRADIO_SUPER_ITALIAN` env var). It is a **personality dial**, not a translation toggle: it changes both UI copy and AI host behavior.

| | Default Super Italian (ON) | Reduced Italian (OFF) |
|---|---|---|
...
| Admin UI | Italian-first operator copy | Italian-first operator copy |
