...
| 2026-09-06 | The admin live deck's sticky backdrop is state-driven, and deviates from the sticky-nav treatment in two named ways | The deck sits where the Sun Glow atmosphere is warmest, so the flat `var(--bg)` fill it used to paint permanently stamped a hard-edged rectangle over the glow and grain — measured as a 17/255 step at its bottom edge. It is now transparent at rest and opaque only while pinned. Two deliberate deviations from the listener sticky-nav spec, both measured rather than assumed: (1) **no `backdrop-filter: blur(12px)`** — behind the admin deck it is espresso on espresso, and blurred vs flat differed by 6/255, which does not pay for a compositing layer on a 271px element that repaints every scroll frame; the blur keeps earning its place on the listener page, where artwork and the brighter `--card*` surfaces pass underneath. (2) **no 1px `--line` bottom border** — the tab bar already ends in its own `--line-strong` rule 16px above the deck's edge, so the hairline rendered as a railroad track; the pinned cue is `box-shadow: 0 6px 16px -12px rgba(0,0,0,.85)` instead, which reads as elevation without a competing line. |
