...
The station has two language modes, set by the operator (admin Engine Room → "Super Italian Mode" toggle, or `MAMMAMIRADIO_SUPER_ITALIAN` env var). It is a **personality dial**, not a translation toggle: it changes both the listener UI copy and the AI host behavior.

| | Default (OFF) | Super Italian (ON) |
|---|---|---|
| Listener UI — headlines, section names, brand idioms (`Stasera in onda`, `Palinsesto`, `Dediche & Saluti`, `Mi`, tricolor) | Italian (always) | Italian |
| Listener UI — buttons, form placeholders, body labels (`Listen Now`, `Tracks in Rotation`, `Your name`) | English | Italian |
| Listener UI — JS dynamic labels (`now`, `min ago`, `Music`, `Banter`) | English | Italian |
| AI host language | Code-switch: English narrative + Italian sprinkles (`ciao`, `dai`, `mamma mia`) | Full Italian, addresses listeners as `amici miei` |
| Admin UI | English (always) | English (always — admin is operator surface) |
| Banter / ad / song-cue *content* generated by hosts | Driven by `[brand].language` (separate concern, not this toggle) | Same |

Swappable strings live in `mammamiradio/web/ui_copy.py` (single source of truth). Admin UI is intentionally not routed through that module — it stays English so the operator surface is always readable, even when the toggle is ON for creative direction.
